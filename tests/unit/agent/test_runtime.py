...
def _make_llm_response_tuple_choices(content="Done.", tool_calls=None):
    return {
        "choices": (
            {
                "message": {
...
class TestAgentRuntimeRun:
    ...
    @patch("owlclaw.agent.runtime.runtime.llm_integration.acompletion")
    async def test_trigger_event_rejects_non_dict_payload(self, mock_llm, tmp_path) -> None:
        mock_llm.return_value = _make_llm_response()
        rt = AgentRuntime(agent_id="bot", app_dir=_make_app_dir(tmp_path))
        await rt.setup()
        with pytest.raises(ValueError, match="payload must be a dictionary"):
    ...
    @patch("owlclaw.agent.runtime.runtime.llm_integration.acompletion")
    async def test_accepts_tuple_choices_payload(self, mock_llm, tmp_path) -> None:
        mock_llm.return_value = _make_llm_response_tuple_choices("Tuple done.")
        rt = AgentRuntime(agent_id="bot", app_dir=_make_app_dir(tmp_path))
        await rt.setup()
        result = await rt.trigger_event("cron")
        assert result["status"] == "completed"
        assert result["final_response"] == "Tuple done."
    ...
    def test_build_tool_decision_reasoning_normalizes_risk_level(self, tmp_path) -> None:
        rt = AgentRuntime(agent_id="bot", app_dir=_make_app_dir(tmp_path))
        ctx = AgentRunContext(agent_id="bot", trigger="cron")
        reasoning = json.loads(
            rt._build_tool_decision_reasoning(
                {"name": "x", "risk_level": " HIGH ", "requires_confirmation": False},
                ctx,
            )
        )
        assert reasoning["risk_level"] == "high"
    ...
    async def test_visible_tools_accepts_mixed_confirmed_capabilities_iterable(
        self, tmp_path
    ) -> None:
        """Runtime should normalize non-string iterable items via str()."""
        rt = AgentRuntime(agent_id="bot", app_dir=_make_app_dir(tmp_path))
        rt.registry = MagicMock()
        rt.registry.list_capabilities.return_value = [
            {"name": "x", "description": "d", "constraints": {}}
        ...
        ctx = AgentRunContext(
            agent_id="bot",
            trigger="cron",
            payload={"confirmed_capabilities": ["x", 7, None, "  "]},
        )
        await rt._get_visible_tools(ctx)
        run_ctx = rt.visibility_filter.filter_capabilities.call_args.args[2]
        assert run_ctx.confirmed_capabilities == {"x", "7"}
    ...
    async def test_heartbeat_payload_tuple_pending_events_skips_checker(
        self, mock_check_events, mock_llm, tmp_path
    ) -> None:
        """Tuple pending_events should be treated as event signal."""
        mock_llm.return_value = _make_llm_response()
        rt = AgentRuntime(agent_id="bot", app_dir=_make_app_dir(tmp_path))
        await rt.setup()
        result = await rt.trigger_event(
            "heartbeat",
            payload={"pending_events": ("webhook:new",)},
        )
        assert result["status"] == "completed"
        mock_check_events.assert_not_called()
        mock_llm.assert_called_once()
