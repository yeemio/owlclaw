...
def test_rollback_current_revision_failure_exits_1(monkeypatch) -> None:
    monkeypatch.setenv("OWLCLAW_DATABASE_URL", "postgresql://u:p@localhost/owlclaw")
    monkeypatch.setattr("owlclaw.cli.db_rollback.get_engine", lambda url: _FakeEngine())
    monkeypatch.setattr(
        "owlclaw.cli.db_rollback._get_current_revision_sync",
        lambda engine: (_ for _ in ()).throw(RuntimeError("db down")),
    )
    monkeypatch.setattr(
        "owlclaw.cli.db_rollback.ScriptDirectory.from_config",
        lambda cfg: _FakeScript([]),
    ...
    with pytest.raises(Exception) as exc_info:
        rollback_command(target="", steps=1, database_url="", dry_run=True, yes=True)
