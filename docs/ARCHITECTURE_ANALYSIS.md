...
### 4.13 决策13：双模接入架构（三段光谱 + 三轨接入）

> **批准日期**: 2026-02-27
> **决策文档**: `docs/DUAL_MODE_ARCHITECTURE_DECISION.md`（完整分析 + 13 项子决策）

#### 三段光谱

OwlClaw 面对的接入对象是一个三段光谱，而非"有 AI / 无 AI"二分法：

| 段位 | 代表 | 接入模式 | OwlClaw 价值 |
|------|------|---------|-------------|
| **A 段** | OpenClaw（有 AI + 有生态） | 协调模式 | 业务接入层 + 企业治理（MCP Server 能力输出） |
| **B 段** | Vibecoding 产物（有 AI + 有瓶颈） | 增强模式 | 治理强化 + 持久执行 + AI-first 升级 |
| **C 段** | 传统 ERP/CRM/HR（无 AI） | 代理模式 | 全栈赋能（SKILL.md + Binding + Agent） |
...
| **Handler 轨** | 增强模式 | `@app.handler` 注册已有函数为 capability |
| **Protocol 轨** | 协调模式 | MCP Server + A2A Agent Card |

三轨不是新增概念，是对 §4.12 Declarative Binding、§4.8 编排框架标准接入、§4.11 Protocol-first 的分类命名。

#### 模式感知事件驱动（D1-R）

| 模式 | 主驱动 |
|------|--------|
| 协调模式 | 协调事件驱动（对方 Agent 请求 + 业务系统事件推送） |
| 增强模式 | 治理事件 + 业务时钟（治理审批请求 + 原系统定时任务） |
| 代理模式 | 业务事件 + 业务时钟（Cron/Webhook/Queue/DB-Change） |

所有模式的事件最终进入 OwlClaw 事件队列，由 Heartbeat 节流器统一调度。

#### 双协议（D8）

- **MCP**：工具接入协议（已有实现，Phase 1-2 重点）
- **A2A**：Agent 间协作协议（Phase 1 仅实现 Agent Card 静态 JSON，Task 管理推迟到有真实协调需求时）

#### MCP 能力输出（面向 OpenClaw 生态）

OwlClaw 的核心能力通过 MCP Server 暴露给外部 Agent（如 OpenClaw）：

| 能力 | MCP Server 工具 |
|------|----------------|
| 业务系统自动接入 | `owlclaw migrate` 生成的业务 MCP Server |
| 预算/限流/熔断 | 治理网关 MCP Server |
| 持久后台任务 | Hatchet 包装 MCP Server |
| 审计 + 权限控制 | Ledger + 可见性过滤 MCP Server |

MCP Server 的工具粒度遵循单一职责、清晰参数原则，适合被 OpenClaw Agent 调用。

#### 开源保护策略

MIT 开源产品代码 + 使用文档；不开源研发过程文档（`.kiro/`、`.cursor/rules/`、架构决策、定位策略）。对外文档侧重"how to use"而非"how to modify"，形成知识壁垒。详见 `docs/POSITIONING.md` §8。

#### 与既有决策的关系

| 新决策 | 关联既有决策 | 关系 |
|--------|-----------|------|
| D4-R 三轨接入 | §4.12 Declarative Binding + §4.8 编排框架接入 + §4.11 Protocol-first | 分类命名，无冲突 |
| D1-R 模式感知事件驱动 | §4.3 事件触发器统一层 | 扩展，按模式区分事件源 |
...
HeartbeatChecker 现状：
- `database`：已实现，查询 Ledger 最近窗口内可重试状态（默认 `error` / `timeout` / `pending`）
- `schedule`：已实现，通过可注入调度客户端读取待执行任务并判断是否到期
- `webhook` / `queue` / `external_api`：保留为扩展点，通过配置开关启用；启用但未接入时记录 warning 并返回 `False`

**最小实现**：`_check_database_events()` — 查询 Ledger 表中最近 N 分钟内是否有可重试记录。选择 DB 而非 Queue 的原因：Ledger 是 OwlClaw 自有组件，无外部依赖，部署门槛最低。  
**补强实现**：`_check_schedule_events()` — 查询调度系统待执行任务，作为 heartbeat 的第二事件源，降低漏检风险。
...
- 查询必须有索引支撑，避免全表扫描
- `_check_schedule_events()` 通过注入式客户端调用，保持运行时与 Hatchet SDK 解耦
- `webhook` / `queue` / `external_api` 仅作为配置化扩展点，不在核心路径硬编码外部依赖
...
### 4.15 决策15：Web Console（治理控制面 + 统一入口）

> **批准日期**: 2026-02-27
> **触发**: 产品交付第一印象评估 + 企业客户评估路径分析
> **关联**: §4.3 治理层、§4.13 双模接入、§4.14 闭环可证明性、§5.4 商业化策略

#### 背景与动机

OwlClaw 已完成 Agent 运行时、治理层、6 类触发器、MCP 能力输出、50+ CLI 命令。但所有交互仅通过 CLI 和 MCP JSON-RPC 暴露，**缺少企业客户评估产品时的第一接触面——Web 控制台**。

竞品分析：
- **Hatchet Dashboard**：覆盖持久执行层（Workflow 可视化、队列指标）——OwlClaw 已集成
- **Langfuse Dashboard**：覆盖 LLM 可观测层（Trace、成本、Prompt 管理）——OwlClaw 已集成
- **CrewAI AMP / Dify Console**：企业级 Agent 平台标杆（Trace、RBAC、部署管理）
- **OpenClaw Control UI**：个人 AI 助手管控面（Vite + Lit SPA，内建在 Gateway 中）

**关键发现**：Hatchet + Langfuse 已覆盖"执行层"和"LLM 层"可观测性，但**没有任何现有工具能展示 OwlClaw 独有的"治理层"**——预算执行、能力可见性过滤、熔断状态、Ledger 审计链路。

#### D15 决策：内建 Web Console

**定位**：治理控制面 + 统一入口（不替代 Hatchet/Langfuse Dashboard，而是聚合 + 深链接）

**技术方案**：FastAPI 后端 + 内嵌 Vite SPA（React + Tailwind + Shadcn/ui）

```
owlclaw/web/                    # Python 后端（REST API + 静态文件服务）
├── api/                        # REST API endpoints
│   ├── overview.py             # 系统健康 + 关键指标
│   ├── agents.py               # Agent 列表 + 身份 + 记忆 + 运行历史
│   ├── governance.py           # 预算/限流/熔断状态 + 策略
│   ├── capabilities.py         # Handlers + Skills + Bindings
│   ├── triggers.py             # 6 类触发器状态 + 执行历史
│   ├── ledger.py               # 审计时间线（分页 + 筛选）
│   └── settings.py             # 配置 + MCP 连接 + 系统信息
├── app.py                      # Starlette/FastAPI 挂载
└── static/                     # Vite 构建产物（内嵌在 Python 包中）

owlclaw-console/                # 前端独立开发目录
├── src/pages/                  # React 页面
...
**部署方式**：
- 静态文件内嵌在 `owlclaw` Python 包中，`pip install owlclaw` 后即可用
- `owlclaw start` 自动启动 Agent 运行时 + Console（同端口）
- 开发时 `pnpm dev` 独立开发前端，生产时打包为静态资源
- 用户无需安装 Node.js
...
| 页面 | 核心功能 | 数据源 | 差异化价值 |
|------|---------|--------|-----------|
| **Overview** | 系统健康、关键指标卡片、告警横幅 | Runtime + Ledger + Governance | 一眼掌握全局 |
| **Agents** | Agent 列表、身份配置、记忆状态、知识库、运行历史 | AgentRuntime + Memory + Knowledge | Agent 生命周期管理 |
| **Governance** | 预算消耗趋势、限流/熔断状态、能力可见性矩阵 | GovernanceEngine + Ledger | **OwlClaw 独有，无替代品** |
| **Capabilities** | Handlers + Skills + Bindings 列表、Schema 查看、调用统计 | CapabilityRegistry + SkillsLoader | 业务接入层可视化 |
| **Triggers** | 6 类触发器状态、执行历史、下次触发时间 | TriggerRegistry | 事件驱动全景 |
| **Ledger** | 执行审计时间线、多维筛选、详情展开 | GovernanceLedger | 决策→执行→结果全链路 |
| **Traces** | 深链接到 Langfuse Dashboard | Langfuse | 聚合入口 |
| **Workflows** | 深链接到 Hatchet Dashboard | Hatchet | 聚合入口 |
| **Settings** | 运行时配置、MCP 连接状态、系统信息 | ConfigManager + MCP Server | 运维诊断 |

**设计原则**：
1. **治理是核心叙事**——Governance 和 Ledger 页面是差异化价值，不是附属功能
2. **深链接而非重造**——Hatchet/Langfuse 的 Dashboard 已经很好，Console 提供聚合入口
3. **只读 MVP → 渐进式可写**——第一版只做展示和监控，降低安全风险和开发量
4. **暗色主题为默认**——企业运维/监控场景，专业且护眼
5. **任务中心设计**——围绕运维人员的任务组织信息，而非数据库实体

**与商业化的关系**：
- **MIT 开源**：Console 核心功能（Overview/Agents/Governance/Capabilities/Triggers/Ledger）
- **企业版**：RBAC、多租户视图、审计日志导出、SSO 集成
- **OwlClaw Cloud**：托管 Console + 多实例聚合 + 历史趋势分析

**技术基础**：
- FastAPI 已是项目依赖（`pyproject.toml`）
- Starlette + uvicorn 已作为可选依赖存在（`extras = api`）
- MCP HTTP Transport 已基于 Starlette 实现（`owlclaw/mcp/http_transport.py`）
- 治理层、Ledger、触发器等数据源 API 已通过 MCP governance_tools 暴露

**隔离架构（防止底层变更波及 Console）**：

Console 依赖 30+ 个底层模块的数据。若直接耦合，任何底层 Spec 的改动都会三层联动（底层→API→前端）。解法是在底层模块和 Console API 之间插入**查询契约层**：

```
底层模块（agent-runtime, governance, triggers, ...）
    │
    ▼
查询契约层 owlclaw/web/contracts.py   ← Protocol 接口，稳定
    │
    ▼
Provider 适配层 owlclaw/web/providers/ ← 底层变更只改这里
    │
    ▼
Console REST API owlclaw/web/api/      ← 只依赖 Protocol，不 import 底层
    │
    ▼
Console 前端（TypeScript 类型从 OpenAPI Schema 自动生成）
```

设计约束：
1. **Console REST API 禁止直接 import 底层模块**——只依赖 `owlclaw/web/contracts.py` 中定义的 Protocol 接口
2. **底层 API 变更时**——只需修改 `owlclaw/web/providers/` 中的适配代码，Contract 和 API 层不变
3. **前端 TypeScript 类型从 OpenAPI Schema 自动生成**——后端 API 返回结构变了，重新生成即可，不手动维护
4. **新增数据维度时**——前端采用 schema-driven 渲染（通用列表/详情组件），新字段自动出现，无需改页面结构
5. **底层 Spec 变更影响评估**——任何底层 Spec 新增/修改 task 时，若影响 Console 查询契约，须在 task 描述中标注 `[console-impact]`

此模式与 `owlclaw/mcp/governance_tools.py` 已采用的 Protocol 解耦模式一致（`GovernanceLedger(Protocol)` 不直接依赖 Ledger 实现类）。
...
| RBAC / 多租户 | 企业版 |
| Web UI / 控制台 | ~~用 Langfuse + Temporal UI~~ → **已纳入 Phase 9（决策15 §4.15）**：内建 Web Console 作为治理控制面 + 统一入口 |
...
> **文档版本**: v4.8（v4.7 + §4.15 Web Console 决策——治理控制面 + 统一入口）
> **创建时间**: 2026-02-10
> **最后更新**: 2026-02-27
> **前置文档**: `DEEP_ANALYSIS_AND_DISCUSSION.md`
> **决策文档**: `docs/DUAL_MODE_ARCHITECTURE_DECISION.md`（2026-02-27 批准）
