...
- [ ] 3.1.3 测试发布流程（先发布到 TestPyPI；2026-02-25 已触发 `Release` run `22389129645`，在 `Validate TestPyPI token` 阶段因 `TEST_PYPI_TOKEN` 为空快速失败）
...
- 外部平台操作待执行（非仓内可自动完成）：
  ...
  - TestPyPI 实际发布验证（当前 run `22389129645` 在 token 前置校验阶段失败，`TEST_PYPI_TOKEN` 为空）
