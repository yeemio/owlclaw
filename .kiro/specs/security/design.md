...
1. 安全审计落库必须遵循 `tenant_id` 前缀索引与时间类型规范，不得使用临时日志替代长期审计记录。
2. 风险门控与脱敏策略为治理链路的一部分，禁止在接入层旁路执行高风险 capability。
...
| 集成位置 | 安全组件 | 调用时机 |
|---------|---------|---------|
| `owlclaw/agent/runtime.py` | InputSanitizer | 构建 prompt 前，对外部输入净化 |
| `owlclaw/agent/runtime.py` | Role Isolation | 构建 messages 时，强制 system/user 分离 |
| `owlclaw/agent/runtime.py` | OutputValidator | LLM 返回 function call 后，执行前验证 |
