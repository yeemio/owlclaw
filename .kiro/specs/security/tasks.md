...
### 1.1 创建安全模块目录
- [ ] 1.1.1 创建 `owlclaw/security/__init__.py`
- [ ] 1.1.2 创建 `owlclaw/security/sanitizer.py`
- [ ] 1.1.3 创建 `owlclaw/security/risk_gate.py`
- [ ] 1.1.4 创建 `owlclaw/security/data_masker.py`
- [ ] 1.1.5 创建 `owlclaw/security/audit.py`
- [ ] 1.1.6 创建 `owlclaw/security/rules.py`
...
- [ ] 2.1.1 实现 `SanitizationRule` 数据模型（pattern, action, description）
- [ ] 2.1.2 实现 `SanitizeResult` 数据模型（original, sanitized, modifications）
- [ ] 2.1.3 实现 `InputSanitizer.__init__` 加载规则（内置 + 自定义）
- [ ] 2.1.4 实现 `InputSanitizer.sanitize()` 方法
- [ ] 2.1.5 实现 `_default_rules()` 内置规则集（至少 10 条已知 injection 模式）
...
### 2.3 单元测试
- [ ] 2.3.1 测试已知 injection 模式检测（至少 10 个攻击向量）
- [ ] 2.3.2 测试自定义规则加载和应用
...
### 3.1 RiskGate 实现
- [ ] 3.1.1 实现 `RiskDecision` 枚举（execute, pause, reject）
- [ ] 3.1.2 实现 `RiskGate.evaluate()` 根据 risk_level 返回决策
- [ ] 3.1.3 实现 `RiskGate.confirm()` 和 `RiskGate.reject()` 确认/拒绝接口
- [ ] 3.1.4 实现确认超时自动取消逻辑
...
### 4.1 DataMasker 实现
- [ ] 4.1.1 实现 `MaskRule` 数据模型（field_pattern, mask_type, replacement）
- [ ] 4.1.2 实现 `DataMasker.mask()` 方法（递归处理 dict）
- [ ] 4.1.3 实现内置脱敏规则（手机号、身份证、银行卡、邮箱）
