...
- [x] 8.1 创建 Docker Compose 配置
  ...
  - [x] 8.1.4 提供 init-db.sql 创建 hatchet/owlclaw 库及用户（详见 `docs/DATABASE_ARCHITECTURE.md`）
  - [x] 8.1.5 部署文档说明两种初始化方式：`owlclaw db init` 与 init-db.sql（见 docs/DATABASE_ARCHITECTURE §5.2）
...
- [x] 11.2 并发执行测试
  ...
  - [x] 11.2.2 目标：所有任务在 30 秒内完成

- [x] 11.3 持久化定时精度测试
  - [x] 11.3.1 测试不同延迟时间的精度
  - [x] 11.3.2 记录实际延迟与预期延迟的差异
