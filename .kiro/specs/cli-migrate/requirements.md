...
1. `owlclaw migrate scan` 命令入口与参数解析（`--openapi` / `--orm` / `--output-mode`）。
2. Declarative Binding 生成能力：`output-mode=binding|both`。
3. `BindingGenerator.generate_from_openapi()` 与 `generate_from_orm()`。
4. 生成结果可通过 `owlclaw skill validate`，并已具备对应单元测试。

以下仍在后续阶段：真正的 Python handler 自动迁移、dry-run 报告、交互式向导、完整配置体系。
