...
### 5. Handler 实迁能力（0/3）
- [ ] 5.1 复用 cli-scan 做 Python 候选函数扫描
- [ ] 5.2 生成可执行 `@app.handler` 注册代码（非 stub）
- [ ] 5.3 缺失类型注解输出 `MANUAL_REVIEW` 汇总
...
### 7. 配置与向导（0/2）
- [ ] 7.1 支持 `.owlclaw-migrate.yaml` 配置加载/校验
- [ ] 7.2 实现交互式迁移向导 `owlclaw migrate init`
