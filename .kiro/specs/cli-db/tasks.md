...
- [x]* 1.1 为配置读取编写属性测试（P1 可选，MVP 跳过）
  - **属性 1：配置优先级一致性**
  - **验证需求：9.1, 9.2, 9.3**
...
- [x] 11. 实现 restore 命令（P2）
  - [x] 11.1 实现 db_restore.py 核心逻辑
  ...
  - [x] 11.3 实现 restore 命令的成功输出和错误处理
    - 显示恢复的表数量和行数
    - 处理恢复失败：显示错误详情和回滚建议
    - _需求：7.6, 7.7_
  
  - [x]* 11.4 为 restore 命令编写单元测试
    - 测试从 SQL 文件恢复
    - 测试从 custom 格式恢复
    - 测试 --clean 参数
    - 测试数据库不为空场景
...
- [x] 13. 实现通用功能和属性测试
  ...
  - [x] 13.3 实现通用的用户体验功能
    - 实现进度指示器：超过 2 秒显示（progress_after(2s)，用于 backup/restore）
    - 实现 Ctrl+C 处理：main() 捕获 KeyboardInterrupt，exit 130
    - 实现 --verbose 参数：backup/restore/check 支持 -v，显示详细进度
