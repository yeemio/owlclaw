...
| 已完成项 | 1) `mionyee-governance-overlay` 已完成（14/14）；2) `mcp-capability-export` 已完成（18/18）；3) `openclaw-skill-pack` 已完成 Task 0/1（8/14）；4) `mionyee-hatchet-migration` 已完成 Task 0~4：任务盘点、迁移生成、双跑回放、cutover 切换与全量分类迁移产物；5) 新增迁移/切换脚本：`scripts/mionyee_apscheduler_to_hatchet.py`、`scripts/mionyee_dual_run_replay.py`、`scripts/mionyee_scheduler_cutover.py`；6) 新增切换决策文件 `.kiro/specs/mionyee-hatchet-migration/cutover_decision.json` 与 `test_hatchet_cutover.py`。 |
| 下一待执行 | 1) `mionyee-hatchet-migration` Task 5.1/5.2/5.3/5.4：恢复、状态查询、回滚开关、端到端验收收口；2) `openclaw-skill-pack` Task 2/3/4：兼容验证、ClawHub 发布流程、教程复现；3) D14-1/D14-2/D14-3 实装。 |
