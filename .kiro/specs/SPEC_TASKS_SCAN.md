...
|------|---|
| 最后更新 | 2026-02-26 |
| 当前批次 | codex-gpt-work 执行循环：release 平台侧可控项落地 + 阻塞再收敛 |
| 批次状态 | **进行中（阶段性收口）**。gateway-runtime-ops 已完成（18/18）；cross-lang-golden-path 已完成（16/16）；release-supply-chain 维持 10/15，但 GitHub 侧可控项已全部到位，阻塞收敛为外部 Trusted Publisher 映射。 |
| 已完成项 | 1) 远端已落地 `main` 分支保护（Lint/Test/Build required checks + PR 审核策略）并刷新 `docs/release/release-policy-audit.json`；2) `pypi-release` environment 已启用 custom branch policies，并新增 `main` 与 `codex-gpt-work` 分支策略；3) 复验 run `22450293930` 仍在 `Publish to TestPyPI` 报 `invalid-publisher`，确认仅剩 PyPI/TestPyPI 发布者映射缺失；4) 新增/刷新 `docs/release/release-supply-chain-audit.json` 作为持续审计证据；5) 新增 `scripts/ops/extract_trusted_publisher_claims.py` 并生成 `docs/release/trusted-publisher-claims.json`，固化 Trusted Publisher 映射模板；6) 复核 owlhub 生产发布目标仓库状态，`gh repo view yeemio/owlhub` 与 `gh repo view owlclaw/owlhub` 均返回 not found，并已回写 `owlhub/tasks.md` 40.4 证据。 |
