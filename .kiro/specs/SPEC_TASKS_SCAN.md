...
|------|---|
| 最后更新 | 2026-02-26 |
| 当前批次 | codex-work 循环：contract-testing API 契约门禁（Task 2.1~2.3） |
| 批次状态 | **进行中**。`contract-testing` 已完成目录基线与 API diff 门禁接入。 |
| 已完成项 | 1) 建立 `tests/contracts/api/` 与 `tests/contracts/mcp/`；2) 建立 `scripts/contract_diff/` 并新增包装入口 `run_contract_diff.py`；3) 新增结构测试 `tests/unit/test_contract_testing_structure.py`；4) 新增 OpenAPI 契约夹具与门禁测试 `tests/contracts/api/test_openapi_contract_gate.py`（additive pass / breaking block）；5) 新增 `docs/protocol/OPENAPI_BREAKING_RULES.md`；6) 新增 PR 门禁工作流 `.github/workflows/contract-gate.yml` 并在 `test_ci_configs.py` 增加校验；7) 更新 `contract-testing/tasks.md` 进度到 `6/19`。 |
| 下一待执行 | 1) 完成 `contract-testing` Task 3.1~3.4（MCP initialize/tools/resources/error 回归用例）；2) 完成 Task 4.1~4.3（对齐矩阵文档、映射填充、评审要求）；3) 跟踪 `test-infra` Task 11.3 远端复跑窗口。 |
| 验收快照 | quick-start ✅(13/13)，complete-workflow ✅(18/18)，architecture-roadmap ✅(13/13)，skill-dx ✅(25/25)，skill-ai-assist ✅(28/28)，progressive-migration ✅(31/31)，skills-quality ✅(27/27)，industry-skills ✅(12/12)，protocol-governance ✅(27/27)，contract-testing 🟡(6/19)，test-infra 🟡(10/11，仅 11.3 待远端复跑)，release 🟡(28/32，外部阻塞)，owlhub 🟡(141/143，仅 40/40.4 未完成)，其余 spec 全部 ✅。 |
