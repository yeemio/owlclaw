...
|------|---|
| 最后更新 | 2026-02-26 |
| 当前批次 | codex-gpt-work 执行循环：release-supply-chain OIDC 链路实跑与根因收敛 |
| 批次状态 | **进行中（阶段性收口）**。gateway-runtime-ops 已完成（18/18）；cross-lang-golden-path 已完成（16/16）；release-supply-chain 维持 10/15，但阻塞根因已从“凭据缺失”收敛为“Trusted Publisher claim 未匹配”。 |
| 已完成项 | 1) 推送并触发 `codex-gpt-work` 分支 release run `22449095206`；2) OIDC 链路通过测试/构建并在 `Publish to TestPyPI` 明确失败为 `invalid-publisher`（`sub=repo:yeemio/owlclaw:ref:refs/heads/codex-gpt-work`）；3) 已将该根因与 claim 证据同步到 `release/tasks.md` 与 `release-supply-chain/tasks.md`。 |
| 下一待执行 | 1) 维护者在 PyPI/TestPyPI 创建匹配 `.github/workflows/release.yml` + 分支 ref 的 Trusted Publisher（先 `codex-gpt-work` 验证，再切 `main`）；2) 完成后重跑 TestPyPI/PyPI 全链路（release-supply-chain Task 4.1/4.2）；3) 发布成功后执行 `pip install owlclaw` smoke，关闭 release 剩余项。 |
...
| 阻塞项 | 1) test-infra Task 11.3：需远端 CI 复跑确认（run `22436813478` 使用旧门槛导致失败，待主分支合并后复验）；2) release/release-supply-chain：主分支 runs `22446541468`、`22447692518`、`22447700064` 仍为旧链路 `TWINE_PASSWORD` 为空导致 TestPyPI `403`；`codex-gpt-work` run `22449095206` 已验证 OIDC 链路并给出 `invalid-publisher`，当前外部阻塞为 PyPI/TestPyPI Trusted Publisher claim 映射未配置；3) owlhub Task 40.4：生产凭据/环境所有权外部阻塞。 |
| 健康状态 | 正常 |
| 连续无进展轮数 | 0 |
