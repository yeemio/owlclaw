...
|------|---|
| 最后更新 | 2026-02-26 |
| 当前批次 | review-work 循环：cross-lang 收口完成 + release-supply-chain 持续推进 |
| 批次状态 | **进行中（阶段性收口）**。gateway-runtime-ops 已完成（18/18）；contract-testing 已完成（19/19）；cross-lang-golden-path 已完成（16/16）；release-supply-chain 推进至 11/15（OIDC/provenance/回滚策略/检查基线与 smoke 验证流程已就位），外部平台配置仍是主阻塞。 |
| 已完成项 | 1) `contract-testing` 全量收口：新增 `docs/protocol/templates/CONTRACT_DIFF_REPORT_TEMPLATE.md`、`docs/protocol/CONTRACT_TESTING_POLICY.md`、`scripts/contract_diff/contract_testing_drill.py` 与单测，完成 5.x/6.x；2) gateway 运维文档与模板：`gateway-rollout-policy.md`、`gateway-runbook.md`、`gateway-slo.md`、`gateway-alerting.md` 与 `docs/ops/templates/GATEWAY_ACCEPTANCE_MATRIX_TEMPLATE.md`；3) gateway 工程收口：新增 `.github/workflows/gateway-ops-gate.yml`、`scripts/gateway_ops_gate.py`、`scripts/gateway_ops_drill.py`，并补齐 `test_gateway_ops_gate.py`、`test_gateway_ops_drill.py`；4) cross-lang 全量收口：新增 Java 客户端与 curl 对照脚本，补齐 `docs/protocol/JAVA_GOLDEN_PATH.md`、`scripts/verify_cross_lang.ps1`、`tests/unit/test_cross_lang_java_assets.py` 与 `tests/unit/test_verify_cross_lang_script.py`，`cross-lang-golden-path/tasks.md` 达成 16/16；5) release-supply-chain 校准：触发并观察 release run `22446541468`（2026-02-26），新增 `docs/release/release-supply-chain-playbook.md`、`docs/release/release-policy-baseline.md`、`docs/release/SUPPLY_CHAIN_POLICY.md`、`scripts/ops/release_policy_audit.py` 与 `docs/release/release-policy-audit.json`；6) release workflow 强化：切换 OIDC 发布、引入 provenance attest（v2）与发布后 smoke 安装路径。 |
| 下一待执行 | 1) release-supply-chain：在 PyPI/TestPyPI 后台完成 Trusted Publisher 绑定（Task 1.1/1.2）并在主分支触发全链路演练（Task 4.1/4.2）；2) release：发布成功后执行 `pip install owlclaw` smoke 关闭剩余发布验收项；3) 跟踪 test-infra Task 11.3 远端复跑窗口；4) 跟踪 owlhub 外部阻塞解除路径。 |
| 验收快照 | quick-start ✅(13/13)，complete-workflow ✅(18/18)，architecture-roadmap ✅(13/13)，skill-dx ✅(25/25)，skill-ai-assist ✅(28/28)，progressive-migration ✅(31/31)，skills-quality ✅(27/27)，industry-skills ✅(12/12)，protocol-governance ✅(27/27)，gateway-runtime-ops ✅(18/18)，contract-testing ✅(19/19)，cross-lang-golden-path ✅(16/16)，release-supply-chain 🟡(11/15)，test-infra 🟡(10/11，仅 11.3 待远端复跑)，release 🟡(28/32，外部阻塞)，owlhub 🟡(141/143，仅 40/40.4 未完成)，其余 spec 全部 ✅。 |
