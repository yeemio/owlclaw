...
|------|---|
| 最后更新 | 2026-02-27 |
| 当前批次 | codex-work 循环：mionyee-hatchet-migration Task 0+1+2 首批落地 |
| 批次状态 | **进行中**。`mionyee-hatchet-migration` 已完成 Task 0/1/2（规范对齐 + 任务盘点 + 迁移工具/单测），进入 Task 3 第一批迁移。 |
| 已完成项 | 1) 规范化 `mionyee-hatchet-migration/tasks.md`，回写 Task 0.1/0.2 勾选；2) 新增盘点文档 `.kiro/specs/mionyee-hatchet-migration/task_inventory.md`，基于等效入口 `config/e2e/scenarios/mionyee-tasks.json` 输出任务清单/复杂度/灰度批次；3) 新增迁移库模块 `owlclaw/integrations/hatchet_migration.py`（任务模型、复杂度分类、canary 选择、APScheduler→Hatchet 渲染、文件输出）；4) 新增迁移脚本 `scripts/mionyee_apscheduler_to_hatchet.py`；5) 生成示例迁移产物 `examples/mionyee-trading/generated_hatchet_tasks.py`；6) 新增 `tests/unit/test_hatchet_migration.py` 覆盖转换逻辑；7) `mionyee-hatchet-migration/tasks.md` 回写 Task 1.1~1.3 与 Task 2.1~2.2 勾选。 |
| 下一待执行 | 1) `mionyee-hatchet-migration` Task 3.1：完善第一批 workflow 产物与接入位点；2) Task 3.2：补双跑模式配置（`SCHEDULER_BACKEND=dual` 等效策略）；3) Task 3.3/3.4：先以集成回放替代 1 周运行，完成一致性校验与 APScheduler 侧切换策略。 |
| 验收快照 | quick-start ✅(13/13)，complete-workflow ✅(18/18)，architecture-roadmap ✅(13/13)，skill-dx ✅(25/25)，skill-ai-assist ✅(28/28)，progressive-migration ✅(31/31)，skills-quality ✅(27/27)，industry-skills ✅(12/12)，protocol-governance ✅(27/27)，contract-testing ✅(19/19)，gateway-runtime-ops ✅(18/18)，cross-lang-golden-path ✅(16/16)，protocol-first-api-mcp ✅(24/24)，test-infra ✅(11/11)，mionyee-governance-overlay ✅(14/14)，mionyee-hatchet-migration 🟡(7/15)，release-supply-chain 🟡(11/15)，release 🟡(28/32，外部阻塞)，owlhub 🟡(141/143，仅 40/40.4 未完成)，Phase 8：mcp-capability-export 🟡(6/18)，openclaw-skill-pack 🟡(0/14)，content-launch 🟡(0/16)，Phase 8.5：D14-1 🟡(0/1)，D14-2 🟡(0/1)，D14-3 🟡(0/1)，其余 spec 全部 ✅。 |
