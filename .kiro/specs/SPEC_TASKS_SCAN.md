...
| **contract-testing** | `.kiro/specs/contract-testing/` | 🟡 三层齐全，进行中（6/19） | API/MCP 契约测试体系（diff 检测、回归、对齐矩阵） |
...
| 批次状态 | **进行中（持续推进）**。protocol-governance 已完成（27/27）；gateway-runtime-ops 完成工程接入（11/18）；cross-lang-golden-path 完成 Java 场景实现（10/16）；contract-testing 保持 API diff 基线与门禁进展（6/19）。 |
| 已完成项 | 1) protocol-governance 收口：`VERSIONING/COMPATIBILITY/ERROR_MODEL/GATE_POLICY`、模板、`contract_diff.py`、`protocol_governance_drill.py` 与对应测试；2) contract-testing 基线与门禁：`tests/contracts/api|mcp/`、`scripts/contract_diff/run_contract_diff.py`、OpenAPI 夹具与 `test_openapi_contract_gate.py`、`OPENAPI_BREAKING_RULES.md`、`contract-gate.yml` 与 CI 配置断言；3) 新增 `scripts/ops/gateway_gate_check.py` 并接入 `release.yml`（Task 4.1）；4) 新增 `scripts/ops/gateway_rollback_executor.py` 并在 `release.yml` 失败路径调用（Task 4.2）；5) 新增 `docs/ops/gateway-alerting-integration.md`（Task 4.3）；6) Java 客户端完成 `trigger/query/error` 场景与超时/重试/幂等示例（Task 1.2/1.3/1.4/2.1/2.2/2.3）。 |
| 下一待执行 | 1) gateway-runtime-ops：完成演练与收口任务（Task 5.1/5.2）并固化证据模板（Task 6.2/6.3）；2) contract-testing：完成 Task 3.1~3.4（MCP 回归）与 Task 4.1~4.3（对齐矩阵/评审要求）；3) release-supply-chain：推进 OIDC Trusted Publishing 与 provenance 归档（Task 1.1~1.3/2.2/2.3）；4) cross-lang-golden-path：完成响应字段一致性与验收产物（Task 3.2/4.3/4.4）；5) 跟踪 test-infra Task 11.3 远端复跑。 |
| 验收快照 | quick-start ✅(13/13)，complete-workflow ✅(18/18)，architecture-roadmap ✅(13/13)，skill-dx ✅(25/25)，skill-ai-assist ✅(28/28)，progressive-migration ✅(31/31)，skills-quality ✅(27/27)，industry-skills ✅(12/12)，protocol-governance ✅(27/27)，gateway-runtime-ops 🟡(11/18)，cross-lang-golden-path 🟡(10/16)，contract-testing 🟡(6/19)，test-infra 🟡(10/11，仅 11.3 待远端复跑)，release 🟡(28/32，外部阻塞)，owlhub 🟡(141/143，仅 40/40.4 未完成)，其余 spec 全部 ✅。 |
