...
| **local-devenv** | `.kiro/specs/local-devenv/` | 🟡 三层齐全，进行中（3/10） | 统一本地开发环境（docker-compose.dev/test/minimal + Makefile + .env.example + DEVELOPMENT.md） |
| **test-infra** | `.kiro/specs/test-infra/` | 🟡 三层齐全，进行中（4/11） | 测试基础设施统一（skip 机制 + unit 纯净化 + 共享 fixtures + 覆盖率分层 + CI 镜像对齐） |
...
| 已完成项 | 1) local-devenv 进度推进到 🟡(3/10)：完成 Task 4（`.env.example` 分区重写与默认值补齐）、Task 8（新增 `docs/DEPLOYMENT.md`）与 Task 9（deploy compose 升级与文档对齐），并落地 Task 1.1~1.3（`docker-compose.test.yml` + `deploy/init-test-db.sql`）与 Task 2.1~2.3（`docker-compose.minimal.yml`）、Task 3.1~3.7（`docker-compose.dev.yml` + `init-db.sql` langfuse 扩展）；2) test-infra 进度推进到 🟡(4/11)：完成 Task 1（跨服务 skip 机制 + markers）、Task 2（integration 默认 postgres 标注与收集验证）、Task 7（覆盖率配置补齐：branch/htmlcov）、Task 8（CI 分层运行 + cov-append）及 Task 4.1（lint job 增加 unit 基线测试）；3) repo-hygiene 收口 ✅(7/7)：完成 Task 1~7 全部验收；4) release 外部可核验社区项已完成并勾选（Discussions/Public/Topics+Description）；5) SPEC_TASKS_SCAN 已更新（Phase 4 + Spec 索引 + Checkpoint）。 |
| 下一待执行 | 1) test-infra Task 3.4（unit 全量 0 skip 验证）与 Task 4（CI unit 纯净检查）；2) local-devenv Task 1.4/2.4/3.8（需 Docker Engine 启动后验证 compose + 命令）；3) release 剩余 7 项（外部平台依赖）；4) owlhub Task 19 架构决策。 |
| 验收快照 | examples ✅(14/14)，cli-migrate ✅(24/24)，ci-setup ✅(12/12)，release 🟡(25/32)，owlhub 🟡(40/42)，capabilities-skills 🟡(108/115)，declarative-binding ✅(26/26)，local-devenv 🟡(3/10)，test-infra 🟡(4/11)，repo-hygiene ✅(7/7)。 |
