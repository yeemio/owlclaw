...
| 批次状态 | **进行中**。`mionyee-hatchet-migration` 已完成（15/15）；`openclaw-skill-pack` 已完成 Task 0/1，Task 2 完成 2.2 子项，Task 3 完成 3.1/3.2，Task 4 完成 4.1/4.2/4.3，当前进度 12/14。 |
| 已完成项 | 1) `mionyee-governance-overlay` 已完成（14/14）；2) `mcp-capability-export` 已完成（18/18）；3) `mionyee-hatchet-migration` 已完成 Task 0~5（15/15）：包含最终验收脚本与报告（gate passed）；4) `openclaw-skill-pack` 已完成基础包、结构/兼容测试、ClawHub 发布前置（PR `openclaw/clawhub#556`）与中英双语一键教程（含复现日志与排障清单）。 |
| 下一待执行 | 1) `openclaw-skill-pack` Task 3.3/3.4：等待 PR 审核合并后验证可搜索/可安装；2) `openclaw-skill-pack` Task 2.1/2.3：在真实 OpenClaw 最新稳定版执行解析与调用验收；3) `content-launch` Task 0：三层文档与 SPEC_TASKS_SCAN 对齐；4) D14-1/D14-2/D14-3 实装。 |
