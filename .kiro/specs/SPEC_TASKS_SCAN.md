...
| triggers-cron | `.kiro/specs/triggers-cron/` | 🟡 三层齐全，进行中（40/92） | cron 触发器 |
...
|------|---|
| 最后更新 | 2026-02-23 |
| 当前批次 | review 循环（agent-tools + hatchet 性能 + cron 手动触发审计） |
| 批次状态 | **完成**。已补齐记忆工具、Hatchet 性能测试，并新增 cron 手动触发 Ledger 审计。 |
| 已完成项 | `owlclaw/agent/tools.py` 新增 `remember/recall` schema 与执行逻辑（含参数校验、Memory 调用、Ledger 记录）；`tests/unit/agent/test_tools.py` 增加记忆工具成功/失败/无结果用例；`tests/unit/test_hatchet_perf.py` 新增 10 并发任务与 schedule 精度测试；`owlclaw/triggers/cron.py` 的 `trigger_now()` 新增 manual trigger Ledger 记录，`tests/unit/triggers/test_cron_validation.py` 补测；回填 agent-tools / integrations-hatchet / triggers-cron 对应 tasks。 |
| 下一待执行 | 继续收口 `integrations-hatchet` 剩余 7.2.3/7.2.4/11.1.2（需真实 Hatchet 环境），并推进 `triggers-cron` 6~16 的 Focus/治理/监控任务。 |
| 验收快照 | `poetry run pytest tests/unit/agent/test_tools.py tests/unit/test_app.py -q` -> `71 passed`；`poetry run pytest tests/unit/test_hatchet_perf.py -q` -> `4 passed`；`poetry run pytest tests/unit/triggers/test_cron_validation.py -q`（本轮新增用例通过）+ 对应 ruff/mypy 均通过。 |
| 阻塞项 | 无。 |
| 健康状态 | 正常 |
| 连续无进展轮数 | 0 |
