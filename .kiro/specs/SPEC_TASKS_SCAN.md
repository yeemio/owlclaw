...
| integrations-langchain | `.kiro/specs/integrations-langchain/` | 🟡 三层齐全，进行中（29/101） | LangChain LLM 后端适配器 + 编排框架集成文档/示例 |
...
| 批次状态 | **进行中**。完成 `integrations-langchain` 核心功能（配置/Schema/错误/Trace/Adapter/重试）与核心检查点（**29/101**）。 |
| 已完成项 | 新增 `owlclaw/integrations/langchain/{__init__,config,schema,errors,trace,adapter,retry}.py`；新增可选依赖声明（`pyproject.toml` extras: `langchain`）；完成 Runnable 注册、执行、超时、重试机制与对应单测/属性测试。 |
| 下一待执行 | `integrations-langchain` Task 11（Governance 集成）→ Task 12（Ledger 集成）→ Task 13（隐私脱敏）。 |
| 验收快照 | `poetry run pytest tests/unit/integrations/test_langchain_config.py tests/unit/integrations/test_langchain_schema.py tests/unit/integrations/test_langchain_errors.py tests/unit/integrations/test_langchain_trace.py tests/unit/integrations/test_langchain_adapter.py tests/unit/integrations/test_langchain_retry.py -q` -> `29 passed`；`poetry run ruff check owlclaw/integrations/langchain tests/unit/integrations/test_langchain_config.py tests/unit/integrations/test_langchain_schema.py tests/unit/integrations/test_langchain_errors.py tests/unit/integrations/test_langchain_trace.py tests/unit/integrations/test_langchain_adapter.py tests/unit/integrations/test_langchain_retry.py` -> `All checks passed!` |
| 阻塞项 | 无。 |
| 健康状态 | 正常 |
| 连续无进展轮数 | 0 |
