...
| **local-devenv** | `.kiro/specs/local-devenv/` | 🟡 三层齐全，进行中（1/10） | 统一本地开发环境（docker-compose.dev/test/minimal + Makefile + .env.example + DEVELOPMENT.md） |
...
| 已完成项 | 1) local-devenv 进度推进到 🟡(1/10)：完成 Task 4（`.env.example` 分区重写与默认值补齐），并落地 Task 1.1~1.3（`docker-compose.test.yml` + `deploy/init-test-db.sql`）与 Task 2.1~2.3（`docker-compose.minimal.yml`）；2) test-infra 🆕(0/11)：requirements + design + tasks 已建立；3) repo-hygiene 进度推进到 🟡(5/7)：完成 Task 1（.gitignore 补齐并验证）、Task 2（nul 清理核验）、Task 3（游离目录核验 + worktree prune）、Task 4（新增 `scripts/README.md`）、Task 6（docs 文件名可访问性核验）；4) release 外部可核验社区项已完成并勾选（Discussions/Public/Topics+Description）；5) SPEC_TASKS_SCAN 已更新（Phase 4 + Spec 索引 + Checkpoint）。 |
| 下一待执行 | 1) local-devenv Task 1.4/2.4（需 Docker Engine 启动后验证 compose + 命令）；2) local-devenv Task 3（`docker-compose.dev.yml`）；3) repo-hygiene Task 5.2（根目录 compose 成为首选入口，依赖 local-devenv 完成）；4) repo-hygiene Task 7（最终验收）；5) release 剩余 7 项（外部平台依赖）；6) owlhub Task 19 架构决策。 |
| 验收快照 | examples ✅(14/14)，cli-migrate ✅(24/24)，ci-setup ✅(12/12)，release 🟡(25/32)，owlhub 🟡(40/42)，capabilities-skills 🟡(108/115)，declarative-binding ✅(26/26)，local-devenv 🟡(1/10)，test-infra 🆕(0/11)，repo-hygiene 🟡(5/7)。 |
