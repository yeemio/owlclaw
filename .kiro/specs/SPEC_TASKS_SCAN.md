...
| Spec 名称 | 路径 | 状态 | 覆盖模块 |
|-----------|------|------|---------|
| capabilities-skills | `.kiro/specs/capabilities-skills/` | 🟡 三层齐全，进行中（114/115） | skills + registry |
...
| 已完成项 | 1) test-infra：Task 3 完成（`1530 passed, 0 skipped`）；Task 9.1~9.3 完成（CI 与本地 test compose 同源 `deploy/init-test-db.sql`）；2) capabilities-skills：新增 Skill prerequisites 加载门控（env/bins/config/python_packages/os）与单测覆盖；3) capabilities-skills：新增 Session Skill Snapshot（run 内技能/能力视图冻结，run 结束释放）与 runtime 单测覆盖；4) capabilities-skills：新增 `owlclaw.yaml` skills.entries enable/disable 门控与单测覆盖；5) capabilities-skills：新增 Token Impact 报告（per-skill/total/dropped）并接入 runtime token budget 裁剪；6) capabilities-skills：新增 Skill 环境变量注入生命周期（run 注入/结束恢复）与 runtime 单测覆盖；7) capabilities-skills：新增 Skill 冲突优先级覆盖（workspace > managed/installed > bundled）与单测覆盖；8) 单测修复：`test_queue_log_security_properties` 改为字段级断言并通过；9) 扫描文档修正：Checkpoint 去除过期 `33/37, 37/56, 137/143` 旧口径。 |
| 下一待执行 | 1) test-infra：Task 4.2（<60s，当前约 `353s`）、Task 6.3、Task 9.4、Task 11（含 CI matrix 与覆盖率门槛最终验收）；2) local-devenv：Task 1.4/2.4/3.8/5.4/7.2/10.*（需 Docker Engine + make 条件）；3) owlhub：Task 19 决策确认与 Task 40.4 外部部署；4) release：PyPI/TestPyPI/GitHub 发布外部动作。 |
| 验收快照 | repo-hygiene ✅(7/7)，local-devenv 🟡(4/10)，test-infra 🟡(7/11)，release 🟡(25/32)，owlhub 🟡(40/42)，capabilities-skills 🟡(114/115)，其余 spec 全部 ✅。 |
