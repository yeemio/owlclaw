...
- [x] CP1 LLMIntegrationConfig 补 mock_mode/mock_responses 字段 → spec: config-propagation-fix
- [x] CP2 create_agent_runtime() 传递 LLM 配置到 Runtime → spec: config-propagation-fix
- [x] CP3 Router default_model 从 integrations.llm.model 派生 → spec: config-propagation-fix
- [x] CP4 Router 未配置 task_type 返回 None → spec: config-propagation-fix
- [x] CP5 ConfigManager 优先级明确（configure() > ENV > YAML > 默认值）→ spec: config-propagation-fix
- [x] CP6 configure() 不得在 start() 之后调用 → spec: config-propagation-fix
- [x] CP7 DEFAULT_RUNTIME_CONFIG 的 model 从 app config 派生 → spec: config-propagation-fix
- [x] CP8 真实 LLM 端到端验证（OpenAI 兼容：DeepSeek 实模通过；Anthropic 无 key 时按约定 skip）→ spec: config-propagation-fix
- [x] CP9 Console Auth 空 Token 绕过修复（`OWLCLAW_REQUIRE_AUTH=true` 时未配置 token 返回 500 + 启动告警）→ spec: config-propagation-fix

**Phase 12.2：安全加固（P0/P1，安全缺陷可导致系统被控制）**

### Phase 16：多 Agent 文件协议闭环（新增）

> **来源**: 2026-03-07 对 workflow runtime 协议的完整闭环审查
> **优先级**: P0（影响统筹、审校、审计、编码的完整回流）
...
- [x] S3 CORS 安全修复（v4 Task 3）→ spec: security-hardening
- [x] S4 HTTP Executor SSRF 防护（v4 Task 4）→ spec: security-hardening
- [x] S5 Unicode 归一化（v4 Task 5）→ spec: security-hardening
- [x] S6 SKILL.md 注入防护（v4 Task 6）→ spec: security-hardening
- [x] S7 Webhook 管理鉴权（v4 Task 7）→ spec: security-hardening
- [x] S8 MCP 认证（v4 Task 8）→ spec: security-hardening
- [x] S9 eval 替换（v4 Task 9）→ spec: security-hardening
- [x] S10 XXE 防护（v4 Task 10）→ spec: security-hardening
- [x] S11 请求体限制（v4 Task 11）→ spec: security-hardening
- [x] S12 SecurityAuditLog 持久化（v4 Task 12）→ spec: security-hardening
- [x] S13 Console API 鉴权 + auth_token 哈希 + 回归（v4 Task 13-15）→ spec: security-hardening
...
### Phase 13：Low 发现收口（2026-03-05 统筹立项）

> **来源**: 2026-03-03-deep-audit-report-v4.md Low findings（#11~#14）
> **优先级**: Low（非发布阻断，建议小批次并行收口）

- [x] L1 Langfuse secret 配置输出脱敏（Finding #11）→ spec: phase13-low-findings
- [x] L2 `_is_select_query` 启发式误判治理（Finding #12）→ spec: phase13-low-findings
- [x] L3 Shadow mode 查询泄露防护（Finding #13）→ spec: phase13-low-findings
- [x] L4 Heartbeat DB I/O 优化（Finding #14）→ spec: phase13-low-findings

### Phase 14：Console 真实浏览器验收（2026-03-05 统筹立项）

> **来源**: `docs/console/BROWSER_TEST_REQUIREMENTS.md` + `docs/console/BROWSER_VERIFICATION_CHECKLIST.md`
> **优先级**: P1（发布质量门）

- [x] B1 Playwright 自动化覆盖主路径（Overview/Governance/Ledger/Agents）→ spec: console-browser-real-e2e
- [x] B2 关键 API 参数与网络断言（筛选/分页/order_by）→ spec: console-browser-real-e2e
...
> **来源**: `docs/review/DEEP_AUDIT_REPORT.md`（27 轮主审 + 扩展轮已完成；Phase 15 对应报告 #1–#29 已修复；报告共 124 条发现，Backlog #30–#44、#56–#124）
...
- [x] D8 Low-8 health_status() 不读私有 _states/_configs（公开 API 或文档）→ spec: audit-deep-remediation
...
| **audit-deep-remediation** | `.kiro/specs/audit-deep-remediation/` | ✅ 三层齐全，已完成（15/15） | 深度审计修复主线：D1-D29 已完成并合入 main；新增 #45-#55 已进入 follow-up 分配。 |
...
| 字段 | 值 |
|------|---|
| 最后更新 | 2026-03-07（深度审计 124 条；Phase 16 #45-#55；codex-work round 47 收口，累计 64 项已完成） |
| 当前批次 | **Phase 16 follow-up**：深度审计 #45-#55。Backlog：#30-#44（未分配）、#56-#124（含第 32 轮 #83-#87、第 33 轮 #88-#94、第 34 轮 #95-#103、第 35 轮 #104-#113、第 36 轮 #114-#116、第 37 轮 #117-#119、第 38 轮 #120-#121、第 39–40 轮 #122-#124 生产代码补审），待后续统筹分配。 |
| 批次状态 | 旧批次 D15-D29 已收口；`codex-work` 已提交 #47/#48/#49/#52/#55（本分支）；`codex-gpt-work` 已提交首批 #45/#46 修复并进入待审；`review-work` 下一轮应审校两编码分支。 |
| 已完成项 | 23) D25 Kafka connect timeout；24) D15-D29 全部修复并合入 main；25) 深度审计主审 27 轮完成；26) 加审补充 #55；27) #45-#55 已完成统筹分配；28) Phase 16 工作状态完成校准；29) `codex-gpt-work` 已提交首批 #45/#46 修复，进入待审；30) 深度审计第 28–31 轮 #56-#82；31) 第 32 轮 #83-#87；32) 第 33 轮 #88-#94；33) 审计复核 Order 45–94；34) 第 34 轮 #95-#103；35) 审计复核 Order 95–103；36) 第 35 轮（scripts 非 workflow）#104-#113；37) 审计复核第四轮 113 条；38) 第 36 轮（tests 关键路径）#114-#116；39) 审计复核第五轮 116 条；40) 第 37 轮（tests 扩展）#117-#119；41) 审计复核第六轮 119 条；42) 第 38 轮（tests web + CWD）#120-#121；43) 审计复核第七轮 121 条；44) 第 39 轮（生产代码补审）#122-#123 入报告；45) 审计复核第八轮：SPEC_TASKS_SCAN 与报告对齐至 124 条、Backlog #56-#124；46) 主 worktree spec 循环：audit-deep-remediation 可选 Task 8（skill_env_allowlist）实现+验收；47) test-infra backlog：pytest-html 测试报告 HTML 输出（dev 依赖 + docs/TESTING.md）；48) test-infra backlog：pytest-xdist 并行测试说明（docs/TESTING.md）；49) ruff F841/F811 修复（server.py exc、capabilities.py 重复 import）；50) docs/TESTING.md 补充覆盖率 HTML 报告命令；51) test-infra tasks.md 最后更新日期校准；52) local-devenv tasks.md 最后更新 2026-03-07；53) test-infra design 补充 Backlog「测试数据工厂」选型说明（factory_boy vs polyfactory）；54) docs/TESTING.md 常用命令补充 unit / not integration marker 示例；55) test-infra design 补充 Backlog「Kafka 本地 mock」实现要点；56) declarative-binding tasks.md 最后更新 2026-03-07；57) docs/TESTING.md 补充 -m \"not slow\" 示例；58) repo-hygiene / contract-testing tasks.md 最后更新 2026-03-07；59) docs/TESTING.md 补充 -x 首次失败即停；60) gateway-runtime-ops / cross-lang-golden-path / protocol-governance / examples tasks.md 最后更新 2026-03-07；61) cli-migrate tasks.md 最后更新 2026-03-07；62) docs/TESTING.md 补充 -v 详细输出示例；63) release / release-supply-chain / console-integration / console-frontend / protocol-first-api-mcp / mcp-server / security / skill-templates / governance tasks.md 最后更新 2026-03-07；64) codex-work spec loop round 47：未提交改动收口确认，Phase 16 批次已交付并审校 APPROVE，相关单元测试 148 通过，无新 assignment；65) codex-work spec loop 本轮回合：未提交改动收口（无）；已 sync main（Already up to date）；Phase 16 分配项 #47/#48/#49/#52/#55 已全部交付，无新 assignment，等待审校；66) codex-work spec loop 续轮：未提交收口（工作区干净）；git merge main 已执行（Already up to date）；无新 assignment，已写 delivery；67) codex-work spec loop round 57：sync main（Already up to date）；无新 assignment；已写 delivery + Checkpoint 更新；68) codex-work spec loop round 58：sync main（Already up to date）；无新 assignment；已写 delivery + Checkpoint 更新；69) codex-work spec loop round 59：sync main（Already up to date）；无新 assignment；已写 delivery + Checkpoint 更新；70) codex-work spec loop round 60：sync main（Already up to date）；无新 assignment；已写 delivery + Checkpoint 更新；71) codex-work spec loop round 61：sync main（Already up to date）；无新 assignment；已写 delivery + Checkpoint 更新；72) codex-work spec loop round 62：sync main（Already up to date）；无新 assignment；已写 delivery + Checkpoint 更新；73) codex-work spec loop round 63：sync main（Already up to date）；无新 assignment；已写 delivery + Checkpoint 更新；74) codex-work spec loop round 64：sync main（Already up to date）；无新 assignment；已写 delivery + Checkpoint 更新；75) codex-work spec loop round 65：sync main（Already up to date）；无新 assignment；已写 delivery + Checkpoint 更新；76) codex-work spec loop round 66：sync main（Already up to date）；无新 assignment；已写 delivery + Checkpoint 更新；77) codex-work spec loop round 67：sync main（Already up to date）；无新 assignment；已写 delivery + Checkpoint 更新；78) codex-work spec loop round 68：sync main（Already up to date）；无新 assignment；已写 delivery + Checkpoint 更新；79) codex-work spec loop round 69：sync main（Already up to date）；无新 assignment；已写 delivery + Checkpoint 更新；80) codex-work spec loop round 70：sync main（Already up to date）；无新 assignment；已写 delivery + Checkpoint 更新；81) codex-work spec loop round 71：sync main（Already up to date）；无新 assignment；已写 delivery + Checkpoint 更新；82) codex-work spec loop round 72：sync main（Already up to date）；无新 assignment；已写 delivery + Checkpoint 更新。 |
| 下一待执行 | 1) `review-work` 审校 `codex-gpt-work` 的 `e8766ea`（#45/#46）与 `codex-work` 的 #47/#48/#49/#52/#55 提交；2) `codex-gpt-work` 继续 #50/#51/#53/#54。 |
| 验收快照 | **Phase 15**：audit-deep-remediation 主线 ✅；**Phase 16**：#45/#46 已提交待审，#47/#48/#49/#52/#55 已提交于 codex-work 待审，#50-#54 待 codex-gpt-work 实施。 |
| 阻塞项 | 无内部阻塞；仅剩外部依赖阻塞项（若有）。 |
| 健康状态 | 🟡 进行中：`codex-gpt-work` 已形成待审提交，`codex-work` 已提交 Phase 16 批次；`review-work` 需尽快消费待审队列。 |
| 连续无进展轮数 | 0 |
| 分支量化进度 | `review-work` 0 commit ahead、工作区干净；`codex-work` 已 merge main 并解决冲突；`codex-gpt-work` 1 commit ahead（`e8766ea`）、工作区干净。 |
| 审校状态 | `audit-deep-remediation` 主线已审校完成；`review-work` 下一轮审校目标为 `codex-gpt-work`（#45/#46）与 `codex-work`（#47/#48/#49/#52/#55），随后再接续 Phase 16 其余修复。 |
