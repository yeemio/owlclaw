...
|------|---|
| 最后更新 | 2026-02-26 |
| 当前批次 | review-work 循环：gateway-runtime-ops 收口 + release/cross-lang 持续推进 |
| 批次状态 | **进行中（阶段性收口）**。gateway-runtime-ops 已完成（18/18）；contract-testing 已完成（19/19）；cross-lang-golden-path 进展至 15/16；release-supply-chain 维持 4/15，待外部平台配置。 |
| 已完成项 | 1) `contract-testing` 全量收口：新增 `docs/protocol/templates/CONTRACT_DIFF_REPORT_TEMPLATE.md`、`docs/protocol/CONTRACT_TESTING_POLICY.md`、`scripts/contract_diff/contract_testing_drill.py` 与单测，完成 5.x/6.x；2) gateway 运维文档与模板：`gateway-rollout-policy.md`、`gateway-runbook.md`、`gateway-slo.md`、`gateway-alerting.md` 与 `docs/ops/templates/GATEWAY_ACCEPTANCE_MATRIX_TEMPLATE.md`；3) gateway 工程收口：新增 `.github/workflows/gateway-ops-gate.yml`、`scripts/gateway_ops_gate.py`、`scripts/gateway_ops_drill.py`，并补齐 `test_gateway_ops_gate.py`、`test_gateway_ops_drill.py`；4) 新增 `scripts/ops/gateway_drill_runner.py` 与两套指标样本（canary-fail/full-success）并完成演练（Task 5.1/5.2）；5) 新增 `docs/ops/gateway-evidence-template.md` 与 `docs/ops/gateway-drill-record.md`，完成阈值与桌面演练固化（Task 6.1/6.2/6.3）；6) 新增 `scripts/cross_lang/compare_response_fields.py` 与 Java/curl fixture，对照通过（Task 3.2）；7) 新增 `docs/protocol/cross-lang-acceptance.md`，固化阈值、验收矩阵与 T+0~T+15 剧本（Task 5.1/5.2/5.3）。 |
| 下一待执行 | 1) release-supply-chain：在 PyPI/TestPyPI 后台完成 Trusted Publisher 绑定（Task 1.1/1.2）并触发全链路演练（Task 4.1/4.2）；2) cross-lang-golden-path：在具备 Maven/JDK 环境后完成 Java 构建执行证据并关闭 Task 4.3；3) release：发布成功后执行 `pip install owlclaw` smoke 关闭剩余发布验收项；4) 跟踪 test-infra Task 11.3 远端复跑窗口。 |
| 验收快照 | quick-start ✅(13/13)，complete-workflow ✅(18/18)，architecture-roadmap ✅(13/13)，skill-dx ✅(25/25)，skill-ai-assist ✅(28/28)，progressive-migration ✅(31/31)，skills-quality ✅(27/27)，industry-skills ✅(12/12)，protocol-governance ✅(27/27)，gateway-runtime-ops ✅(18/18)，contract-testing ✅(19/19)，cross-lang-golden-path 🟡(15/16)，release-supply-chain 🟡(4/15)，test-infra 🟡(10/11，仅 11.3 待远端复跑)，release 🟡(28/32，外部阻塞)，owlhub 🟡(141/143，仅 40/40.4 未完成)，其余 spec 全部 ✅。 |
