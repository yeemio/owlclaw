...
| 已完成项 | 1) 远端已落地 `main` 分支保护（Lint/Test/Build required checks + PR 审核策略）并刷新 `docs/release/release-policy-audit.json`；2) `pypi-release` environment 已启用 custom branch policies，并新增 `main` 与 `codex-gpt-work` 分支策略；3) 复验 run `22450293930` 仍在 `Publish to TestPyPI` 报 `invalid-publisher`，确认仅剩 PyPI/TestPyPI 发布者映射缺失；4) 新增/刷新 `docs/release/release-supply-chain-audit.json` 作为持续审计证据；5) 复核 owlhub 生产发布目标仓库状态，`gh repo view yeemio/owlhub` 与 `gh repo view owlclaw/owlhub` 均返回 not found，并已回写 `owlhub/tasks.md` 40.4 证据。 |
