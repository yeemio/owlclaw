...
| **mionyee-governance-overlay** | `.kiro/specs/mionyee-governance-overlay/` | 🟡 三层齐全，进行中（6/12） | Mionyee 治理叠加（预算/限流/熔断包裹 LLM 调用） |
...
|------|---|
| 最后更新 | 2026-02-27 |
| 当前批次 | review-work 审校循环：Phase 8.1 GovernanceProxy + Phase 8.2 MCP/A2A/migrate-mcp 并行收口 |
| 批次状态 | **进行中**。`mionyee-governance-overlay` 已完成 Task 0/1（6/12），`mcp-capability-export` 已完成 Task 0/2/3/4.1/4.2/5（15/18）；下一步推进 Mionyee 接入胶水、MCP Spike、Task 4.3 与端到端验收。 |
| 已完成项 | 1) D1-R 至 D14-3 决策已签署，`ARCHITECTURE_ANALYSIS.md` 升级至 v4.7 并新增 §4.14；2) Phase 7 全部 spec 审校通过合并；3) `release` spec 三层文档按 28/32 实况规范化，外部阻塞口径固定；4) 新增 `owlclaw/governance/proxy.py`，实现 `GovernanceProxy`（预算/限流/熔断/审计）与 `from_config`、passthrough；5) 新增 `tests/unit/governance/test_proxy.py` 并覆盖预算拦截、限流拒绝、熔断触发、降级直通、配置解析；6) 完成治理 MCP 工具：`governance_budget_status`/`governance_audit_query`/`governance_rate_limit_status`；7) 完成持久任务 MCP 工具：`task_create`/`task_status`/`task_cancel`；8) 完成 A2A Agent Card：新增 `create_agent_card_app` 与 `/.well-known/agent.json` 端点；9) 扩展 `owlclaw migrate` 支持 `--output-mode mcp`，可从 OpenAPI 生成 `mcp_tools/*.json` 工具定义；10) 更新 migrate 配置校验与 CLI help/argparse 选项支持 `mcp`；11) 新增并通过单测：`test_governance_tools.py`、`test_task_tools.py`、`test_agent_card.py`、`test_binding_generator.py`、`test_migrate_scan_cli.py`、`test_migrate_config_cli.py`、`test_cli_main.py`。 |
| 下一待执行 | 1) `mionyee-governance-overlay` Task 2：补 Mionyee 侧接入胶水与配置样例并新增集成测试；2) `mionyee-governance-overlay` Task 3/4：补 Ledger CLI 查询链路与治理判定延迟基线；3) `mcp-capability-export` Task 1：完成 MCP 架构 Spike（HTTP/stdio 体验、延迟、demo、接入文档）；4) `mcp-capability-export` Task 4.3：补生成 MCP 工具的集成测试（OpenClaw 可连接路径）；5) `mcp-capability-export` Task 6：端到端验收；6) D14-1/D14-2/D14-3 实装（运行模式契约文档、闭环 CI 验收、Heartbeat DB 事件源与 SLO 测试）；7) 并行跟踪外部阻塞：release-supply-chain Trusted Publisher 与 owlhub 40.4。 |
| 验收快照 | quick-start ✅(13/13)，complete-workflow ✅(18/18)，architecture-roadmap ✅(13/13)，skill-dx ✅(25/25)，skill-ai-assist ✅(28/28)，progressive-migration ✅(31/31)，skills-quality ✅(27/27)，industry-skills ✅(12/12)，protocol-governance ✅(27/27)，contract-testing ✅(19/19)，gateway-runtime-ops ✅(18/18)，cross-lang-golden-path ✅(16/16)，protocol-first-api-mcp ✅(24/24)，test-infra ✅(11/11)，release-supply-chain 🟡(11/15)，release 🟡(28/32，外部阻塞)，owlhub 🟡(141/143，仅 40/40.4 未完成)，Phase 8：mionyee-governance-overlay 🟡(6/12)，mionyee-hatchet-migration 🟡(0/15)，mcp-capability-export 🟡(15/18)，openclaw-skill-pack 🟡(0/14)，content-launch 🟡(0/16)，Phase 8.5：D14-1 🟡(0/1)，D14-2 🟡(0/1)，D14-3 🟡(0/1)，其余 spec 全部 ✅。 |
