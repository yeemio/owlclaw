...
| integrations-langchain | `.kiro/specs/integrations-langchain/` | 🟡 三层齐全，进行中（72/101） | LangChain LLM 后端适配器 + 编排框架集成文档/示例 |
...
| 批次状态 | **进行中**。完成 `integrations-langchain` 核心功能 + 流式输出 + 配置模板与示例 + 端到端测试 + 文档 + 容错/健康检查（**72/101**）。 |
| 已完成项 | 新增 `owlclaw/integrations/langchain/{__init__,config,schema,errors,trace,adapter,retry,privacy,version}.py`；新增可选依赖声明（`pyproject.toml` extras: `langchain` + `packaging`）；完成 Runnable 注册、执行、超时、重试、权限校验、执行审计记录、输入输出脱敏、App 集成、依赖版本校验、流式事件输出、降级逻辑、健康检查；新增 `config/langchain.example.yaml`、`examples/langchain/` 5 个示例和示例校验测试；新增 `tests/integration/test_langchain_integration.py` 覆盖完整流程/错误场景/性能；新增 `docs/integrations/langchain.md`、`docs/integrations/langchain_migration.md` 并更新架构文档。 |
| 下一待执行 | `integrations-langchain` Task 24（监控指标）→ Task 25（最终检查点）。 |
| 验收快照 | `poetry run pytest tests/unit/test_app_langchain.py tests/unit/integrations/test_langchain_config.py tests/unit/integrations/test_langchain_schema.py tests/unit/integrations/test_langchain_errors.py tests/unit/integrations/test_langchain_trace.py tests/unit/integrations/test_langchain_adapter.py tests/unit/integrations/test_langchain_retry.py tests/unit/integrations/test_langchain_governance_ledger.py tests/unit/integrations/test_langchain_privacy.py tests/unit/integrations/test_langchain_version.py tests/unit/integrations/test_langchain_stream.py tests/unit/integrations/test_langchain_examples.py tests/unit/integrations/test_langchain_health.py tests/integration/test_langchain_integration.py -q` -> `55 passed`；`poetry run ruff check owlclaw/app.py owlclaw/integrations/langchain tests/unit/test_app_langchain.py tests/unit/integrations/test_langchain_config.py tests/unit/integrations/test_langchain_schema.py tests/unit/integrations/test_langchain_errors.py tests/unit/integrations/test_langchain_trace.py tests/unit/integrations/test_langchain_adapter.py tests/unit/integrations/test_langchain_retry.py tests/unit/integrations/test_langchain_governance_ledger.py tests/unit/integrations/test_langchain_privacy.py tests/unit/integrations/test_langchain_version.py tests/unit/integrations/test_langchain_stream.py tests/unit/integrations/test_langchain_examples.py tests/unit/integrations/test_langchain_health.py tests/integration/test_langchain_integration.py examples/langchain` -> `All checks passed!` |
| 阻塞项 | 无。 |
| 健康状态 | 正常 |
| 连续无进展轮数 | 0 |
