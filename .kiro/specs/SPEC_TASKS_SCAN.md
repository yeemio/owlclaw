...
| **console-frontend** | `.kiro/specs/console-frontend/` | 🟡 三层齐全，进行中（4/10） | Console 前端 SPA（9 页面 + React + Tailwind + Shadcn/ui + 暗色主题 + WebSocket + 测试） |
...
|------|---|
| 最后更新 | 2026-03-02 |
| 当前批次 | Phase 9 Web Console 开发进行中：`console-frontend` 已完成 Task 0（脚手架）+ Task 1（布局/路由/API Client/TanStack Query）+ Task 2（Overview）+ Task 3（Governance：预算趋势/熔断状态/可见性矩阵/migration_weight/Skills 排行），并通过 `npm run build` 验收；`console-backend-api` 仍为 0/11，`console-integration` 仍为 0/5。 |
