...
| **console-integration** | `.kiro/specs/console-integration/` | ✅ 三层齐全，已完成（5/5） | Console 集成（`owlclaw start` 挂载 + CLI + 构建流程 + 打包 + 集成测试） |
...
|------|---|
| 最后更新 | 2026-03-02 |
| 当前批次 | Phase 9 实现收口 + 审校修复：console-frontend 10/10 ✅（`useApi.ts` 对齐后端契约映射 + WS 类型修正为 `overview/triggers/ledger` + 新增契约测试；`npm run test` 7 passed，`npm run build` 通过），console-integration 5/5 ✅（挂载器/CLI/打包/集成测试 5 passed）。 |
...
| 下一待执行 | **Phase 9 后端收口**：1) `codex-work` 持续推进 `console-backend-api` 的契约一致性和审校闭环；2) review-work 完成最终复审并合并到 `main`。**Phase 8 外部阻塞项**：3) `content-launch` 真实数据 + 外部发布；4) `openclaw-skill-pack` PR 合并；5) `release-supply-chain` Trusted Publisher 配置。 |
