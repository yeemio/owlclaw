...
| Spec 名称 | 路径 | 状态 | 覆盖模块 |
|-----------|------|------|---------|
| capabilities-skills | `.kiro/specs/capabilities-skills/` | 🟡 三层齐全，进行中（109/115） | skills + registry |
...
| 已完成项 | 1) test-infra：Task 3 完成（`1530 passed, 0 skipped`）；Task 9.1~9.3 完成（CI 与本地 test compose 同源 `deploy/init-test-db.sql`）；2) capabilities-skills：新增 Skill prerequisites 加载门控（env/bins/config/python_packages/os）与单测覆盖；3) 单测修复：`test_queue_log_security_properties` 改为字段级断言并通过；4) 扫描文档修正：Checkpoint 去除过期 `33/37, 37/56, 137/143` 旧口径。 |
...
| 验收快照 | repo-hygiene ✅(7/7)，local-devenv 🟡(4/10)，test-infra 🟡(7/11)，release 🟡(25/32)，owlhub 🟡(40/42)，capabilities-skills 🟡(109/115)，其余 spec 全部 ✅。 |
