...
| **local-devenv** | `.kiro/specs/local-devenv/` | 🟡 三层齐全，进行中（7/10） | 统一本地开发环境（docker-compose.dev/test/minimal + Makefile + .env.example + DEVELOPMENT.md） |
...
|------|---|
| 最后更新 | 2026-02-25 |
| 当前批次 | codex-gpt-work：local-devenv Task 3.8 实测收口 + docker-compose.dev 健康检查修复（Langfuse/hatchet） |
| 批次状态 | **进行中**。本轮已完成 local-devenv Task 3（含 3.8 实测），并修复 compose 健康检查变量展开问题。 |
| 已完成项 | 1) local-devenv：Task 3.8 完成（`docker compose -f docker-compose.dev.yml --profile full up -d` 后 `owlclaw-db/redis/hatchet-lite/langfuse` 全部 healthy）；2) `docker-compose.dev.yml` 新增 `SERVER_MSGQUEUE_KIND=postgres`，并修复 Langfuse healthcheck 为 `$$HOSTNAME`；3) 资产单测更新并通过（`tests/unit/test_local_devenv_assets.py`，`10 passed`）。 |
| 下一待执行 | 1) local-devenv：Task 5.4、Task 7.2、Task 10.1/10.3/10.4；2) test-infra：Task 4.2（<60s）、Task 6.3、Task 9.4、Task 11；3) owlhub：Task 19 决策确认与 Task 40.4 外部部署；4) release：PyPI/TestPyPI/GitHub 发布外部动作。 |
| 验收快照 | repo-hygiene ✅(7/7)，local-devenv 🟡(7/10)，test-infra 🟡(7/11)，release 🟡(25/32)，owlhub 🟡(40/42)，capabilities-skills 🟡(108/115)，其余 spec 全部 ✅。 |
