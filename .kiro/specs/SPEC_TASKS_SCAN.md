...
| 已完成项 | 1) 决策文档 13 项全部签署（D1-R 至 D13）；2) POSITIONING.md v2.0.0 + ARCHITECTURE_ANALYSIS.md v4.6（§4.13）；3) Phase 7 全部 spec 审校通过合并；4) release OIDC 证据链补齐 + 分支保护规则集落地；5) `release` spec 三层文档按 28/32 实况规范化（完成项勾选对齐 + Trusted Publisher 外部阻塞保留）；6) 新增 `owlclaw.mcp.governance_tools.register_governance_mcp_tools` 并实现 `governance_budget_status`、`governance_audit_query`、`governance_rate_limit_status`；7) 新增单测 `tests/unit/mcp/test_governance_tools.py` 并通过定向回归；8) `mcp-capability-export/tasks.md` 已回写 Task 0/2 勾选状态。 |
