...
| 已完成项 | 1) local-devenv 进度推进到 🟡(3/10)：完成 Task 4（`.env.example` 分区重写与默认值补齐）、Task 8（新增 `docs/DEPLOYMENT.md`）与 Task 9（deploy compose 升级与文档对齐），并落地 Task 1.1~1.3（`docker-compose.test.yml` + `deploy/init-test-db.sql`）与 Task 2.1~2.3（`docker-compose.minimal.yml`）、Task 3.1~3.7（`docker-compose.dev.yml` + `init-db.sql` langfuse 扩展）；2) test-infra 进度推进到 🟡(3/11)：完成 Task 1（跨服务 skip 机制 + markers）、Task 2（integration 默认 postgres 标注与收集验证）与 Task 7（覆盖率配置补齐：branch/htmlcov）；3) repo-hygiene 收口 ✅(7/7)：完成 Task 1~7 全部验收；4) release 外部可核验社区项已完成并勾选（Discussions/Public/Topics+Description）；5) SPEC_TASKS_SCAN 已更新（Phase 4 + Spec 索引 + Checkpoint）。 |
| 下一待执行 | 1) test-infra Task 3（unit 层外部依赖清理）；2) test-infra Task 8（CI 分层运行）；3) local-devenv Task 1.4/2.4/3.8（需 Docker Engine 启动后验证 compose + 命令）；4) release 剩余 7 项（外部平台依赖）；5) owlhub Task 19 架构决策。 |
| 验收快照 | examples ✅(14/14)，cli-migrate ✅(24/24)，ci-setup ✅(12/12)，release 🟡(25/32)，owlhub 🟡(40/42)，capabilities-skills 🟡(108/115)，declarative-binding ✅(26/26)，local-devenv 🟡(3/10)，test-infra 🟡(3/11)，repo-hygiene ✅(7/7)。 |
