...
| **mionyee-hatchet-migration** | `.kiro/specs/mionyee-hatchet-migration/` | 🟡 三层齐全，进行中（10/15） | Mionyee 调度迁移（APScheduler → Hatchet 持久执行） |
...
|------|---|
| 最后更新 | 2026-02-27 |
| 当前批次 | codex-work 循环：mionyee-hatchet-migration Task 3 首批双跑落地 |
| 批次状态 | **进行中**。`mionyee-hatchet-migration` 已完成 Task 0/1/2/3.1/3.2/3.3（生成 + 双跑配置 + 回放一致性），剩余 Task 3.4 与 Task 4/5。 |
| 已完成项 | 1) 规范化 `mionyee-hatchet-migration/tasks.md`，回写 Task 0.1/0.2 勾选；2) 新增盘点文档 `.kiro/specs/mionyee-hatchet-migration/task_inventory.md`，基于等效入口 `config/e2e/scenarios/mionyee-tasks.json` 输出任务清单/复杂度/灰度批次；3) 新增迁移库模块 `owlclaw/integrations/hatchet_migration.py`（任务模型、复杂度分类、canary 选择、APScheduler→Hatchet 渲染、双跑回放比较）；4) 新增迁移脚本 `scripts/mionyee_apscheduler_to_hatchet.py` 与双跑回放脚本 `scripts/mionyee_dual_run_replay.py`；5) 生成迁移产物 `examples/mionyee-trading/generated_hatchet_tasks.py` 与 `generated_hatchet_tasks_canary.py`；6) 新增测试 `tests/unit/test_hatchet_migration.py`、`tests/unit/test_mionyee_hatchet_migration.py`、`tests/integration/test_mionyee_hatchet_migration.py`；7) 产出回放报告 `.kiro/specs/mionyee-hatchet-migration/dual_run_replay_report.md` 与 JSON 报告，all/canary 匹配率均 100%。 |
| 下一待执行 | 1) `mionyee-hatchet-migration` Task 3.4：在示例接入层完成切换路径（确认一致后关闭 APScheduler 侧）；2) Task 4：扩展到有状态/链式/剩余任务批次；3) Task 5：恢复测试、状态查询、回滚与端到端验收。 |
| 验收快照 | quick-start ✅(13/13)，complete-workflow ✅(18/18)，architecture-roadmap ✅(13/13)，skill-dx ✅(25/25)，skill-ai-assist ✅(28/28)，progressive-migration ✅(31/31)，skills-quality ✅(27/27)，industry-skills ✅(12/12)，protocol-governance ✅(27/27)，contract-testing ✅(19/19)，gateway-runtime-ops ✅(18/18)，cross-lang-golden-path ✅(16/16)，protocol-first-api-mcp ✅(24/24)，test-infra ✅(11/11)，mionyee-governance-overlay ✅(14/14)，mionyee-hatchet-migration 🟡(10/15)，release-supply-chain 🟡(11/15)，release 🟡(28/32，外部阻塞)，owlhub 🟡(141/143，仅 40/40.4 未完成)，Phase 8：mcp-capability-export 🟡(6/18)，openclaw-skill-pack 🟡(0/14)，content-launch 🟡(0/16)，Phase 8.5：D14-1 🟡(0/1)，D14-2 🟡(0/1)，D14-3 🟡(0/1)，其余 spec 全部 ✅。 |
