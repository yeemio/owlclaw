...
| integrations-langchain | `.kiro/specs/integrations-langchain/` | 🟡 三层齐全，进行中（48/101） | LangChain LLM 后端适配器 + 编排框架集成文档/示例 |
...
| 批次状态 | **进行中**。完成 `integrations-langchain` 核心功能 + Governance/Ledger + 隐私脱敏 + OwlClawApp 集成 + 版本检查（**48/101**）。 |
| 已完成项 | 新增 `owlclaw/integrations/langchain/{__init__,config,schema,errors,trace,adapter,retry,privacy,version}.py`；新增可选依赖声明（`pyproject.toml` extras: `langchain` + `packaging`）；完成 Runnable 注册、执行、超时、重试、权限校验、执行审计记录、输入输出脱敏、App 集成与依赖版本校验。 |
| 下一待执行 | `integrations-langchain` Task 17（流式输出）→ Task 18/19（配置模板与示例）→ Task 20（端到端测试）。 |
| 验收快照 | `poetry run pytest tests/unit/test_app_langchain.py tests/unit/integrations/test_langchain_config.py tests/unit/integrations/test_langchain_schema.py tests/unit/integrations/test_langchain_errors.py tests/unit/integrations/test_langchain_trace.py tests/unit/integrations/test_langchain_adapter.py tests/unit/integrations/test_langchain_retry.py tests/unit/integrations/test_langchain_governance_ledger.py tests/unit/integrations/test_langchain_privacy.py tests/unit/integrations/test_langchain_version.py -q` -> `45 passed`；`poetry run ruff check owlclaw/app.py owlclaw/integrations/langchain pyproject.toml tests/unit/test_app_langchain.py tests/unit/integrations/test_langchain_config.py tests/unit/integrations/test_langchain_schema.py tests/unit/integrations/test_langchain_errors.py tests/unit/integrations/test_langchain_trace.py tests/unit/integrations/test_langchain_adapter.py tests/unit/integrations/test_langchain_retry.py tests/unit/integrations/test_langchain_governance_ledger.py tests/unit/integrations/test_langchain_privacy.py tests/unit/integrations/test_langchain_version.py` -> `All checks passed!` |
| 阻塞项 | 无。 |
| 健康状态 | 正常 |
| 连续无进展轮数 | 0 |
