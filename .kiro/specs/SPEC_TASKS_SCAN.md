...
| 批次状态 | **进行中（已开工）**。protocol-governance 已完成（27/27）；gateway-runtime-ops 完成策略与运维文档基线（8/18）；cross-lang-golden-path 完成 Java 基础骨架与验证脚本（4/16）；contract-testing 已完成目录基线与 API diff 门禁（6/19）。 |
| 已完成项 | 1) protocol-governance 收口：`VERSIONING/COMPATIBILITY/ERROR_MODEL/GATE_POLICY`、模板、`contract_diff.py`、`protocol_governance_drill.py` 与对应测试；2) gateway/runtime/cross-lang 基线：`gateway-rollout-policy`、`gateway-runbook`、`gateway-slo`、Java 示例骨架与脚本；3) contract-testing 基线与门禁：`tests/contracts/api|mcp/`、`scripts/contract_diff/run_contract_diff.py`、OpenAPI 夹具与 `test_openapi_contract_gate.py`、`OPENAPI_BREAKING_RULES.md`、`contract-gate.yml` 与 CI 配置断言。 |
| 下一待执行 | 1) gateway-runtime-ops：接入 pipeline gate / rollback executor / dashboard 告警联动（Task 4.x）并安排演练记录（Task 5.x/6.3）；2) contract-testing：完成 Task 3.1~3.4（MCP 回归）与 Task 4.1~4.3（对齐矩阵/评审要求）；3) release-supply-chain：推进 OIDC workflow 改造与 provenance 归档（Task 1.3/2.2/2.3）；4) 跟踪 test-infra Task 11.3 远端复跑。 |
| 验收快照 | quick-start ✅(13/13)，complete-workflow ✅(18/18)，architecture-roadmap ✅(13/13)，skill-dx ✅(25/25)，skill-ai-assist ✅(28/28)，progressive-migration ✅(31/31)，skills-quality ✅(27/27)，industry-skills ✅(12/12)，protocol-governance ✅(27/27)，gateway-runtime-ops 🟡(8/18)，cross-lang-golden-path 🟡(4/16)，contract-testing 🟡(6/19)，test-infra 🟡(10/11，仅 11.3 待远端复跑)，release 🟡(28/32，外部阻塞)，owlhub 🟡(141/143，仅 40/40.4 未完成)，其余 spec 全部 ✅。 |
