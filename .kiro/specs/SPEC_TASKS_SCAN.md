...
| integrations-langchain | `.kiro/specs/integrations-langchain/` | 🟡 三层齐全，进行中（61/101） | LangChain LLM 后端适配器 + 编排框架集成文档/示例 |
...
| 批次状态 | **进行中**。完成 `integrations-langchain` 核心功能 + 流式输出 + 配置模板与示例 + 端到端测试（**61/101**）。 |
...
| 下一待执行 | `integrations-langchain` Task 21（文档）→ Task 22/23（容错与健康检查）→ Task 24（监控指标）。 |
