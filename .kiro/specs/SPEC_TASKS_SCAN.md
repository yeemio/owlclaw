...
| 已完成项 | 1) 修改并推送 `release.yml`：`release` job 固定 `environment: pypi-release`；2) 触发 run `22449361552` 并确认 OIDC claim 已稳定为 `sub=repo:yeemio/owlclaw:environment:pypi-release`（含 `environment=pypi-release`）；3) 该 run 仍在 TestPyPI `Publish` 返回 `invalid-publisher`，已把证据回写到 `release/tasks.md` 与 `release-supply-chain/tasks.md`。 |
