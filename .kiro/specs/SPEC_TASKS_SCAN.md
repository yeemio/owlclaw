...
| 已完成项 | 1) D1-R 至 D14-3 决策已签署，`ARCHITECTURE_ANALYSIS.md` 升级至 v4.7 并新增 §4.14；2) `mionyee-governance-overlay` 已完成（14/14）；3) `mcp-capability-export` 已完成（18/18）；4) `mionyee-hatchet-migration` 已完成 Task 0/1/2/3.1/3.2/3.3：任务盘点、迁移生成、双跑配置与回放一致性；5) 新增迁移工具与脚本：`owlclaw/integrations/hatchet_migration.py`、`scripts/mionyee_apscheduler_to_hatchet.py`、`scripts/mionyee_dual_run_replay.py`；6) 新增回放报告 `.kiro/specs/mionyee-hatchet-migration/dual_run_replay_report.md` 与 JSON 报告，all/canary 匹配率 100%。 |
