...
| **progressive-migration** | `.kiro/specs/progressive-migration/` | 🟡 三层齐全，进行中（12/25） | 渐进式迁移 migration_weight（MigrationGate + 风险评估 + 审批队列 + Ledger 增强 + CLI） |
...
|------|---|
| 最后更新 | 2026-02-26 |
| 当前批次 | spec loop：quick-start + complete-workflow 收口，progressive-migration Task 1/2 完成 |
| 批次状态 | **进行中**。progressive-migration 已完成 MigrationGate + RiskAssessor 基础能力（12/25），待接入审批队列/ledger/runtime/CLI。 |
| 已完成项 | 1) quick-start 全量完成（示例 + 文档 + 测试）；2) complete-workflow 全量完成（4 skills + 4 handlers + app + README + 测试）；3) progressive-migration 新增 `migration_gate.py`、`risk_assessor.py` 与对应单测（7 passed），并完成 Task 1/2 勾选。 |
| 下一待执行 | 1) codex-gpt-work：progressive-migration Task 3/4/5/6（审批队列 + ledger 增强 + runtime 集成 + CLI）→ industry-skills（0/12）；2) codex-work：test-infra 剩余 + architecture-roadmap → skill-dx P1 → skills-quality；3) release + owlhub 外部凭据（人工）；4) skill-ai-assist P1 在 skill-dx P1 完成后分配。 |
| 验收快照 | quick-start ✅(13/13)，complete-workflow ✅(18/18)，architecture-roadmap 🆕(0/13)，skill-dx 🆕(0/25，P1:18 P2:7)，skill-ai-assist 🆕(0/22，P1:16 P2:6)，progressive-migration 🟡(12/25)，skills-quality 🆕(0/21)，industry-skills 🆕(0/12，已降级为搜索推荐)，test-infra 🟡(7/11)，release 🟡(25/32)，owlhub 🟡(137/143)，其余 spec 全部 ✅。 |
