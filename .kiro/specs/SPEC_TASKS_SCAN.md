...
| **governance-hardening** | `.kiro/specs/governance-hardening/` | 🟡 三层齐全，进行中（8/26） | 治理层加固：Ledger 索引 + UUID PK + fallback 路径 + MODEL_PRICING + QualityStore 索引 + env.py 导入 + session 缓存 + DB 异常包装 + SSL + Cron 去重 |
...
| 字段 | 值 |
|------|---|
| 最后更新 | 2026-03-03（Phase 12 runtime-robustness 收口 + governance-hardening Task 1/2） |
| 当前批次 | **Phase 12（codex-gpt）**：runtime-robustness Task 1-19 已完成；governance-hardening 已完成 Task 1（Ledger tenant 前缀索引）与 Task 2（WebhookIdempotency UUID 主键，含迁移+测试）。 |
| 批次状态 | **Phase 11 功能实现已完成（F1-F10 ✅）**，F11 全量回归待 review-work 执行。**Phase 12 进行中**：codex-gpt 已完成 runtime-robustness（41/41）并推进 governance-hardening（8/26）；codex-work 执行 config-propagation-fix + security-hardening。Phase 8 外部阻塞持续跟踪。 |
...
| 下一待执行 | **Phase 12（codex-gpt）**：governance-hardening Task 3/4（fallback 路径 + MODEL_PRICING）→ Task 5（SkillQualityStore 索引）→ Task 6（env.py 导入 OwlHub 模型）。 |
