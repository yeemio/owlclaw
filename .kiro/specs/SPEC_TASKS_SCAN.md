...
| 已完成项 | 1) `cli-migrate` 任务进度归一：`tasks.md` 与 `SPEC_TASKS_SCAN` 同步为 12/24；2) `owlhub` 进度修正为 40/42，并补充 Task 19 范围决策说明；3) `examples`/`release`/`mcp-server` 进度概览与实际勾选数对齐（0/12、0/32、12/12）；4) 清理 `cli-db`/`cli-scan` 中“测试可跳过”旧表述并统一为“发布前必须完成”；5) 规范化 `owlhub` spec 的代码路径引用（`owlhub/...` → `owlclaw/owlhub/...`）以对齐当前仓库结构；6) 修正 `security` spec 集成路径为真实代码位置（`owlclaw/agent/runtime/runtime.py`、`owlclaw/governance/visibility.py`）；7) 修正 `integrations-hatchet` 中数据库架构文档路径引用（`DATABASE_ARCHITECTURE.md` → `docs/DATABASE_ARCHITECTURE.md`）；8) 修正 `cli-migrate/tasks.md` 章节编号重复（结构对齐 `.kiro/SPEC_DOCUMENTATION_STANDARD.md`）；9) 定向验证：`pytest tests/unit/test_cli_db.py tests/unit/test_cli_db_properties.py tests/integration/test_cli_db_workflows.py -q`（22 passed）。 |
