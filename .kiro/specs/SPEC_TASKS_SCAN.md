...
| **lite-mode-e2e** | `.kiro/specs/lite-mode-e2e/` | 🟡 三层齐全，进行中（17/42） | Lite Mode 端到端体验修复：mock LLM 统一 + Heartbeat 直通 + 日志 + --once 决策 + pgvector 延迟导入 + Quick Start 重写 + CLI/API 降级 + Model 配置传递 + Router 默认行为 |
...
|------|---|
| 最后更新 | 2026-03-03 |
| 当前批次 | **Phase 11 lite-mode-e2e**：Lite Mode 端到端体验修复（0/42 → 17/42，Task 5-8 已完成）。codex-work 负责 Task 1-4（核心链路：mock LLM + heartbeat + 日志 + --once）；codex-gpt-work 已完成 Task 5-8（pgvector + Quick Start + CLI + API）。 |
