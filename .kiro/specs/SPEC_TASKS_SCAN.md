...
| **gateway-runtime-ops** | `.kiro/specs/gateway-runtime-ops/` | 🟡 三层齐全，进行中（8/18） | 网关发布与运维（灰度、回滚、SLO、运行手册） |
| **contract-testing** | `.kiro/specs/contract-testing/` | 🟡 三层齐全，进行中（3/19） | API/MCP 契约测试体系（diff 检测、回归、对齐矩阵） |
| **release-supply-chain** | `.kiro/specs/release-supply-chain/` | 🟡 三层齐全，待实施（0/15） | 发布供应链安全（OIDC、attestation、发布门禁） |
| **cross-lang-golden-path** | `.kiro/specs/cross-lang-golden-path/` | 🟡 三层齐全，进行中（4/16） | 跨语言落地路径（Java/curl 场景化接入与验收） |
...
|------|---|
| 最后更新 | 2026-02-26 |
| 当前批次 | codex-gpt-work 执行循环：gateway-runtime-ops + cross-lang-golden-path 首批落地 |
| 批次状态 | **进行中（已开工）**。protocol-governance 已完成（27/27）；gateway-runtime-ops 完成策略与运维文档基线（8/18）；cross-lang-golden-path 完成 Java 基础骨架与验证脚本（4/16）；contract-testing 完成目录与脚本基线（3/19）。 |
| 已完成项 | 1) protocol-governance 收口：`VERSIONING/COMPATIBILITY/ERROR_MODEL/GATE_POLICY`、模板、`contract_diff.py`、`protocol_governance_drill.py` 与对应测试；2) 新增 `docs/ops/gateway-rollout-policy.md`、`gateway-runbook.md`、`gateway-slo.md`；3) 新增 Java 示例骨架 `examples/cross_lang/java/`（`pom.xml` + `Main.java` + `GatewayClient.java`）及 `scripts/cross_lang/*.sh`、`docs/protocol/JAVA_GOLDEN_PATH.md`、`scripts/verify_cross_lang.ps1`；4) contract-testing 基线：`tests/contracts/api|mcp/`、`scripts/contract_diff/run_contract_diff.py`、`tests/unit/test_contract_testing_structure.py`。 |
| 下一待执行 | 1) gateway-runtime-ops：接入 pipeline gate / rollback executor / dashboard 告警联动（Task 4.x）并安排演练记录（Task 5.x/6.3）；2) contract-testing：完成 Task 2.1~2.3（OpenAPI diff + breaking 规则映射 + PR 门禁）与 Task 3.1~3.4（MCP 回归）；3) release-supply-chain：推进 OIDC workflow 改造与 provenance 归档（Task 1.3/2.2/2.3）；4) 跟踪 test-infra Task 11.3 远端复跑。 |
| 验收快照 | quick-start ✅(13/13)，complete-workflow ✅(18/18)，architecture-roadmap ✅(13/13)，skill-dx ✅(25/25)，skill-ai-assist ✅(28/28)，progressive-migration ✅(31/31)，skills-quality ✅(27/27)，industry-skills ✅(12/12)，protocol-governance ✅(27/27)，gateway-runtime-ops 🟡(8/18)，cross-lang-golden-path 🟡(4/16)，contract-testing 🟡(3/19)，test-infra 🟡(10/11，仅 11.3 待远端复跑)，release 🟡(28/32，外部阻塞)，owlhub 🟡(141/143，仅 40/40.4 未完成)，其余 spec 全部 ✅。 |
