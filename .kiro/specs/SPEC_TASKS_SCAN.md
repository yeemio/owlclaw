...
|------|---|
| 最后更新 | 2026-02-27 |
| 当前批次 | codex-gpt-work 执行循环：release 规范化对齐 + 外部阻塞固化 |
| 批次状态 | **进行中（阶段性收口）**。gateway-runtime-ops 已完成（18/18）；cross-lang-golden-path 已完成（16/16）；release-supply-chain 维持 10/15，GitHub 侧可控项已完成并完成 spec 文档对齐，阻塞收敛为外部 Trusted Publisher 映射。 |
| 已完成项 | 1) 远端已落地 `main` 分支保护（Lint/Test/Build required checks + PR 审核策略）并刷新 `docs/release/release-policy-audit.json`；2) `pypi-release` environment 已启用 custom branch policies，并新增 `main` 与 `codex-gpt-work` 分支策略；3) 复验 run `22450293930` 仍在 `Publish to TestPyPI` 报 `invalid-publisher`，确认仅剩 PyPI/TestPyPI 发布者映射缺失；4) 新增/刷新 `docs/release/release-supply-chain-audit.json` 与 `docs/release/trusted-publisher-claims.json`；5) 完成 `release` spec 规范化：`requirements/design/tasks` 从 token-only 语义对齐到 Trusted Publishing 优先；6) 修复 PR CI 中 release workflow 旧断言（`tests/unit/ci/test_ci_configs.py`、`tests/unit/test_release_assets.py`）；7) 修复 Python 3.10 兼容性回归（`examples/complete_workflow/handlers/report.py` 使用 `timezone.utc`、`scripts/release_consistency_check.py` 增加 `tomllib/tomli` 回退），对应失败用例本地通过；8) 修复 Test workflow 长时挂起风险：新增 `concurrency.cancel-in-progress`、job `timeout-minutes: 45`、unit/integration 命令级 `timeout`；9) 复核 owlhub 生产发布目标仓库状态，`gh repo view yeemio/owlhub` 与 `gh repo view owlclaw/owlhub` 均返回 not found，并已回写 `owlhub/tasks.md` 40.4 证据；10) 补充 integration 测试阶段诊断参数（`-vv`、`--maxfail=5`、`--durations=20`），提升 CI 卡顿与慢测定位效率。 |
| 下一待执行 | 1) 维护者在 TestPyPI/PyPI 创建 Trusted Publisher：`owner=yeemio`、`repo=owlclaw`、`workflow=.github/workflows/release.yml`、`environment=pypi-release`；2) 完成后重跑 TestPyPI/PyPI 全链路（release-supply-chain Task 4.1/4.2）；3) 发布成功后执行 `pip install owlclaw` smoke，关闭 release 剩余项。 |
...
| 阻塞项 | 1) test-infra Task 11.3：需远端 CI 复跑确认（run `22436813478` 使用旧门槛导致失败，待主分支合并后复验）；2) release/release-supply-chain：主分支旧链路 runs `22446541468`、`22447692518`、`22447700064` 因空 `TWINE_PASSWORD` 在 TestPyPI `403`；`codex-gpt-work` OIDC 链路 runs `22449095206`、`22449361552`、`22450293930` 在 environment/branch policy 与分支保护到位后仍 `invalid-publisher`，当前唯一外部阻塞为 PyPI/TestPyPI Trusted Publisher 映射缺失；3) owlhub Task 40.4：生产凭据/环境所有权外部阻塞，且目标仓库 `yeemio/owlhub` / `owlclaw/owlhub` 当前均不存在（`gh repo view` not found）。 |
| 健康状态 | 正常 |
| 连续无进展轮数 | 0 |
