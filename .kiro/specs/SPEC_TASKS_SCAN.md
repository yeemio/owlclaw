...
| **declarative-binding** | `.kiro/specs/declarative-binding/` | 🟡 三层齐全，进行中（14/26） | 声明式工具绑定（Task 0~13 已完成：契约/schema + Resolver/Registry + HTTP/Queue/SQL Executor + BindingTool/Ledger + Skills 自动注册 + CLI 验证扩展 + Shadow 报告链路 + 安全/治理集成 + SKILL.md 最小模式/简化 tools） |
...
|------|---|
| 最后更新 | 2026-02-24 |
| 当前批次 | coding loop（codex-work: declarative-binding Task 13） |
| 批次状态 | **进行中（本批）**。`declarative-binding` 已推进至 14/26。 |
| 已完成项 | 1) 完成 Task 13：默认最小可用 SKILL.md（name+description+body）与 `skill init --minimal/--no-minimal`；2) 新增 `tools` 简化语法规范化（`tools`/`tools_schema` 统一提取，支持 `param: string` 与对象写法）；3) 运行时与 `skill validate` 共用规范化逻辑并补单测；4) 新增 `docs/SKILL_WRITING_GUIDE.md`。 |
| 下一待执行 | `codex-work`：进入 `declarative-binding` Task 14（Reference Implementation 示例）。 |
| 验收快照 | 本批验证：`pytest tests/unit/test_cli_skill.py tests/unit/test_skills.py tests/integration/test_bindings_skills_loader_integration.py -q` + `ruff check`（本轮执行结果见提交说明）。 |
| 阻塞项 | 无。 |
| 健康状态 | 正常 |
| 连续无进展轮数 | 0 |
