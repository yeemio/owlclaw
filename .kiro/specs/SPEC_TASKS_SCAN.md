...
| **mcp-capability-export** | `.kiro/specs/mcp-capability-export/` | 🟡 三层齐全，进行中（17/18） | MCP 能力输出（治理/持久任务/业务接入作为 MCP Server 暴露 + A2A Agent Card） |
...
|------|---|
| 最后更新 | 2026-02-27 |
| 当前批次 | review-work 审校循环：mionyee-governance-overlay 已收口 + mcp-capability-export 收尾推进 |
| 批次状态 | **进行中**。`mionyee-governance-overlay` 已完成 Task 0~4（14/14，已完成）；`mcp-capability-export` 已完成 Task 0/1/2/3/4/5（17/18），仅剩 Task 6 端到端验收。 |
| 已完成项 | 1) D1-R 至 D14-3 决策已签署，`ARCHITECTURE_ANALYSIS.md` 升级至 v4.7 并新增 §4.14；2) Phase 7 全部 spec 审校通过合并；3) `release` spec 三层文档按 28/32 实况规范化，外部阻塞口径固定；4) 新增 `owlclaw/governance/proxy.py` 并实现 `GovernanceProxy`（预算/限流/熔断/审计）+ `from_config` + passthrough；5) 新增 Mionyee 接入胶水 `examples/mionyee-trading/ai/client.py` 与 `examples/mionyee-trading/owlclaw.yaml`，并完成预算/限流/熔断/审计与 p99<10ms 验收；6) 新增 `owlclaw ledger query` 命令与 `tests/unit/test_cli_ledger.py`；7) 完成治理 MCP 工具、持久任务 MCP 工具、A2A Agent Card；8) 扩展 `owlclaw migrate --output-mode mcp`，并新增 `register_generated_mcp_tools` 与生成→注册→调用集成测试链路；9) 完成 MCP 架构 Spike：新增 HTTP transport `create_mcp_http_app`、集成压测 `tests/integration/test_mcp_spike_transport_integration.py`、可运行 demo `scripts/mcp_spike_demo.py`、接入文档 `docs/mcp/OPENCLAW_SPIKE.md`。 |
| 下一待执行 | 1) `mionyee-hatchet-migration` Task 0/1/2：Spec 对齐、APScheduler 任务盘点与迁移映射、迁移工具与最小灰度链路；2) `mcp-capability-export` Task 6：端到端验收（OpenClaw 接入与持久任务行为）；3) D14-1/D14-2/D14-3 实装（运行模式契约文档、闭环 CI 验收、Heartbeat DB 事件源与 SLO 测试）；4) 并行跟踪外部阻塞：release-supply-chain Trusted Publisher 与 owlhub 40.4。 |
| 验收快照 | quick-start ✅(13/13)，complete-workflow ✅(18/18)，architecture-roadmap ✅(13/13)，skill-dx ✅(25/25)，skill-ai-assist ✅(28/28)，progressive-migration ✅(31/31)，skills-quality ✅(27/27)，industry-skills ✅(12/12)，protocol-governance ✅(27/27)，contract-testing ✅(19/19)，gateway-runtime-ops ✅(18/18)，cross-lang-golden-path ✅(16/16)，protocol-first-api-mcp ✅(24/24)，test-infra ✅(11/11)，mionyee-governance-overlay ✅(14/14)，release-supply-chain 🟡(11/15)，release 🟡(28/32，外部阻塞)，owlhub 🟡(141/143，仅 40/40.4 未完成)，Phase 8：mionyee-hatchet-migration 🟡(0/15)，mcp-capability-export 🟡(17/18)，openclaw-skill-pack 🟡(0/14)，content-launch 🟡(0/16)，Phase 8.5：D14-1 🟡(0/1)，D14-2 🟡(0/1)，D14-3 🟡(0/1)，其余 spec 全部 ✅。 |
