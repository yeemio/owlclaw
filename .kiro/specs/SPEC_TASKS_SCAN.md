...
> **最后更新**: 2026-03-04（Phase 12 codex-work：governance-hardening Task 2 fail-close 默认策略落地）
...
| **governance-hardening** | `.kiro/specs/governance-hardening/` | 🟡 三层齐全，进行中（6/33） | 治理层加固：已完成 Task 2（Visibility fail_policy 默认 `close` + fail-open 文档限定 dev/test + 单测）；剩余 Ledger 索引/UUID PK/fallback 路径/MODEL_PRICING/QualityStore 索引/env.py 导入/session 缓存/DB 异常包装/SSL/Cron 去重 |
