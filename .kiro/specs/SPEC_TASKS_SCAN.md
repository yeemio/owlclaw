...
| cli-migrate | `.kiro/specs/cli-migrate/` | 🟡 三层齐全，进行中（12/24） | AI 辅助迁移工具（binding 生成与 output-mode 已落地；handler 实迁、dry-run、报告、向导待完成） |
...
| 下一待执行 | 1) codex-gpt-work 推进 cli-migrate（12/24）：handler 实迁、dry-run、迁移报告与配置向导；2) codex-work 推进 ci-setup Task 11/12 或转向 release；3) 在具备生产凭据后执行 owlhub 40.4 真实生产部署验收。 |
| 验收快照 | 当前：ci-setup 🟡(10/12)，declarative-binding ✅(26/26)，owlhub 🟡(41/42)，examples ✅(12/12)，cli-migrate 🟡(12/24)，release 🟡(0/32)，全仓测试门禁待验证（新 pyproject.toml 配置）。 |
