...
| **declarative-binding** | `.kiro/specs/declarative-binding/` | 🟡 三层齐全，进行中（9/26） | 声明式工具绑定（Task 0~8 已完成：契约/schema + Resolver/Registry + HTTP/Queue Executor + BindingTool/Ledger + Skills 自动注册 + CLI 验证扩展） |
...
|------|---|
| 最后更新 | 2026-02-24 |
| 当前批次 | coding loop（codex-work: declarative-binding Task 8） |
| 批次状态 | **进行中（本批）**。`declarative-binding` 已推进至 9/26。 |
| 已完成项 | 1) 完成 Task 8：新增 `QueueBindingExecutor`（active/shadow 模式、headers_mapping 模板替换）；2) 复用 `integrations.queue_adapters`，为 Kafka/Mock 适配器补充统一 `publish()` 接口；3) 新增单测覆盖 active publish、shadow 拦截、配置校验。 |
| 下一待执行 | `codex-work`：进入 `declarative-binding` Task 9（SQLBinding Executor）。 |
| 验收快照 | 本批验证：`pytest tests/unit/capabilities/test_bindings_queue_executor.py tests/unit/capabilities/test_bindings_http_executor.py tests/unit/capabilities/test_bindings_schema.py tests/unit/capabilities/test_bindings_executor.py -q`（16 passed）+ `pytest tests/unit/capabilities/test_bindings_tool.py tests/integration/test_bindings_http_executor_integration.py tests/integration/test_bindings_skills_loader_integration.py -q`（7 passed）+ `ruff check` 通过。 |
| 阻塞项 | 无。 |
| 健康状态 | 正常 |
| 连续无进展轮数 | 0 |
