...
|------|---|
| 最后更新 | 2026-02-27 |
| 当前批次 | codex-gpt-work：Phase 8.2 `mcp-capability-export` Task 4（`migrate --output-mode mcp`） |
| 批次状态 | **进行中**。`mcp-capability-export` 已完成 Task 0/2/3/4.1/4.2/5（15/18），剩余 Task 1（Spike）、Task 4.3（集成测试）、Task 6（端到端验收）。 |
| 已完成项 | 1) D1-R 至 D14-3 决策已签署；2) `docs/ARCHITECTURE_ANALYSIS.md` 升级至 v4.7 并新增 §4.14；3) Phase 7 全部 spec 审校通过合并；4) `release` spec 三层文档按 28/32 实况规范化，外部阻塞口径固定；5) 完成治理 MCP 工具：`governance_budget_status`/`governance_audit_query`/`governance_rate_limit_status`；6) 完成持久任务 MCP 工具：`task_create`/`task_status`/`task_cancel`；7) 完成 A2A Agent Card：新增 `create_agent_card_app` 与 `/.well-known/agent.json` 端点；8) 扩展 `owlclaw migrate` 支持 `--output-mode mcp`，可从 OpenAPI 生成 `mcp_tools/*.json` 工具定义；9) 更新 migrate 配置校验与 CLI help/argparse 选项支持 `mcp`；10) 新增并通过单测：`tests/unit/cli_migrate/test_binding_generator.py`、`tests/unit/cli_migrate/test_migrate_scan_cli.py`、`tests/unit/cli_migrate/test_migrate_config_cli.py`、`tests/unit/test_cli_main.py` 相关用例。 |
| 下一待执行 | 1) `mcp-capability-export` Task 1：完成 MCP 架构 Spike（HTTP/stdio 体验、延迟、demo、接入文档）；2) `mcp-capability-export` Task 4.3：补生成 MCP 工具的集成测试（OpenClaw 可连接路径）；3) D14-1/D14-2/D14-3 实装：运行模式契约文档、闭环 CI 验收、Heartbeat DB 事件源与 SLO 测试；4) 并行跟踪外部阻塞：release-supply-chain Trusted Publisher 与 owlhub 40.4。 |
| 验收快照 | quick-start ✅(13/13)，complete-workflow ✅(18/18)，architecture-roadmap ✅(13/13)，skill-dx ✅(25/25)，skill-ai-assist ✅(28/28)，progressive-migration ✅(31/31)，skills-quality ✅(27/27)，industry-skills ✅(12/12)，protocol-governance ✅(27/27)，contract-testing ✅(19/19)，gateway-runtime-ops ✅(18/18)，cross-lang-golden-path ✅(16/16)，protocol-first-api-mcp ✅(24/24)，test-infra ✅(11/11)，release-supply-chain 🟡(11/15)，release 🟡(28/32，外部阻塞)，owlhub 🟡(141/143，仅 40/40.4 未完成)，Phase 8：mionyee-governance-overlay 🟡(0/12)，mionyee-hatchet-migration 🟡(0/15)，mcp-capability-export 🟡(15/18)，openclaw-skill-pack 🟡(0/14)，content-launch 🟡(0/16)，Phase 8.5：D14-1 🟡(0/1)，D14-2 🟡(0/1)，D14-3 🟡(0/1)，其余 spec 全部 ✅。 |
