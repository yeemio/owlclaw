...
| **mionyee-hatchet-migration** | `.kiro/specs/mionyee-hatchet-migration/` | 🟡 三层齐全，进行中（7/15） | Mionyee 调度迁移（APScheduler → Hatchet 持久执行） |
| **mcp-capability-export** | `.kiro/specs/mcp-capability-export/` | ✅ 三层齐全，已完成（18/18） | MCP 能力输出（治理/持久任务/业务接入作为 MCP Server 暴露 + A2A Agent Card） |
...
|------|---|
| 最后更新 | 2026-02-27 |
| 当前批次 | review-work 审校循环：Phase 8.1 调度迁移推进 + Phase 8.5 预备 |
| 批次状态 | **进行中**。`mionyee-governance-overlay` 已完成 Task 0~4（14/14）；`mcp-capability-export` 已完成 Task 0~6（18/18）；`mionyee-hatchet-migration` 已完成 Task 0/1/2（7/15），进入 Task 3 第一批迁移。 |
| 已完成项 | 1) D1-R 至 D14-3 决策已签署，`ARCHITECTURE_ANALYSIS.md` 升级至 v4.7 并新增 §4.14；2) Phase 7 全部 spec 审校通过合并；3) `release` spec 三层文档按 28/32 实况规范化，外部阻塞口径固定；4) `mionyee-governance-overlay` 全量完成并验收（14/14）；5) `mcp-capability-export` 完成治理/持久任务/业务 MCP 工具、A2A Agent Card、HTTP/stdio Spike 与 OpenClaw 端到端验收（18/18）；6) `mionyee-hatchet-migration` 完成 Task 0/1/2：规范对齐、任务盘点文档、迁移库 `owlclaw/integrations/hatchet_migration.py`、迁移脚本 `scripts/mionyee_apscheduler_to_hatchet.py`、示例产物与单测 `tests/unit/test_hatchet_migration.py`。 |
| 下一待执行 | 1) `mionyee-hatchet-migration` Task 3.1~3.4：第一批 workflow 接入、双跑模式、一致性回放与 APScheduler 侧切换；2) `openclaw-skill-pack` Task 0/1：三层文档与包结构/元数据落地；3) D14-1/D14-2/D14-3 实装（运行模式契约文档、闭环 CI 验收、Heartbeat DB 事件源与 SLO 测试）；4) 并行跟踪外部阻塞：release-supply-chain Trusted Publisher 与 owlhub 40.4。 |
| 验收快照 | quick-start ✅(13/13)，complete-workflow ✅(18/18)，architecture-roadmap ✅(13/13)，skill-dx ✅(25/25)，skill-ai-assist ✅(28/28)，progressive-migration ✅(31/31)，skills-quality ✅(27/27)，industry-skills ✅(12/12)，protocol-governance ✅(27/27)，contract-testing ✅(19/19)，gateway-runtime-ops ✅(18/18)，cross-lang-golden-path ✅(16/16)，protocol-first-api-mcp ✅(24/24)，test-infra ✅(11/11)，mionyee-governance-overlay ✅(14/14)，mcp-capability-export ✅(18/18)，mionyee-hatchet-migration 🟡(7/15)，release-supply-chain 🟡(11/15)，release 🟡(28/32，外部阻塞)，owlhub 🟡(141/143，仅 40/40.4 未完成)，Phase 8：openclaw-skill-pack 🟡(0/14)，content-launch 🟡(0/16)，Phase 8.5：D14-1 🟡(0/1)，D14-2 🟡(0/1)，D14-3 🟡(0/1)，其余 spec 全部 ✅。 |
