...
> **最后更新**: 2026-03-04（Phase 12 codex-work：governance-hardening Task 1/2/3/6 持续收口）
...
| **governance-hardening** | `.kiro/specs/governance-hardening/` | 🟡 三层齐全，进行中（16/33） | 治理层加固：已完成 Task 1（API trigger 可配置 token bucket 限流：per-tenant + per-endpoint + 429 测试）、Task 2（Visibility fail_policy 默认 `close` + fail-open 文档限定 dev/test + 单测）、Task 3（Budget 原子预约 + TTL 退款 + 并发测试）与 Task 6（Ledger fallback_log_path 配置化 + app 透传 + 单测）；剩余 Ledger 索引/UUID PK/MODEL_PRICING/QualityStore 索引/env.py 导入/session 缓存/DB 异常包装/SSL/Cron 去重 |
