...
|------|---|
| 最后更新 | 2026-02-28 |
| 当前批次 | codex-work 循环：Phase 8.1 收口后 spec 规范化对齐 |
| 批次状态 | **已完成**。`mionyee-governance-overlay` 与 `mionyee-hatchet-migration` 均已完成并通过验收；本轮完成文档口径规范化（目标态 vs 仓库等效态）与 checkpoint 校准。 |
| 已完成项 | 1) 规范化 `mionyee-hatchet-migration/tasks.md`，回写 Task 0.1/0.2 勾选；2) 新增盘点文档 `.kiro/specs/mionyee-hatchet-migration/task_inventory.md`，基于等效入口 `config/e2e/scenarios/mionyee-tasks.json` 输出任务清单/复杂度/灰度批次；3) 新增迁移库模块 `owlclaw/integrations/hatchet_migration.py`（任务模型、复杂度分类、canary 选择、APScheduler→Hatchet 渲染、双跑回放比较、按复杂度分批输出）；4) 新增迁移脚本 `scripts/mionyee_apscheduler_to_hatchet.py`、双跑回放脚本 `scripts/mionyee_dual_run_replay.py`、cutover 脚本 `scripts/mionyee_scheduler_cutover.py`、最终验收脚本 `scripts/mionyee_migration_acceptance.py`；5) 生成迁移产物 `generated_hatchet_tasks.py`、`generated_hatchet_tasks_canary.py` 与按复杂度拆分的 `generated_hatchet_tasks_simple_cron.py`/`stateful_cron.py`/`chained.py`；6) 产出回放报告、切换决策和最终验收报告（`final_acceptance_report.json/.md`，gate passed）；7) 新增测试 `test_hatchet_migration.py`、`test_mionyee_hatchet_migration.py`、`test_hatchet_cutover.py`、`test_hatchet_acceptance.py`、`test_mionyee_migration_acceptance.py`。 |
| 下一待执行 | 1) 等待统筹分配新的 codex-work spec；2) 若统筹指定，可切入 Phase 8.5 D14（D14-1/2/3）或 release-supply-chain 外部阻塞解除后的实操验收。 |
| 验收快照 | quick-start ✅(13/13)，complete-workflow ✅(18/18)，architecture-roadmap ✅(13/13)，skill-dx ✅(25/25)，skill-ai-assist ✅(28/28)，progressive-migration ✅(31/31)，skills-quality ✅(27/27)，industry-skills ✅(12/12)，protocol-governance ✅(27/27)，contract-testing ✅(19/19)，gateway-runtime-ops ✅(18/18)，cross-lang-golden-path ✅(16/16)，protocol-first-api-mcp ✅(24/24)，test-infra ✅(11/11)，mionyee-governance-overlay ✅(14/14)，mionyee-hatchet-migration ✅(15/15)，release-supply-chain 🟡(11/15)，release 🟡(28/32，外部阻塞)，owlhub 🟡(141/143，仅 40/40.4 未完成)，Phase 8：mcp-capability-export 🟡(6/18)，openclaw-skill-pack 🟡(0/14)，content-launch 🟡(0/16)，Phase 8.5：D14-1 🟡(0/1)，D14-2 🟡(0/1)，D14-3 🟡(0/1)，其余 spec 全部 ✅。 |
