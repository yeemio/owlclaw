...
|------|---|
| 最后更新 | 2026-02-26 |
| 当前批次 | codex-work 循环：test-infra 3.10 兼容阻塞定位与修复完成，等待远端复跑收口 |
| 批次状态 | **进行中**。本地已修复 Python 3.10 兼容性问题，待 CI matrix 复跑确认 Task 11.3。 |
| 已完成项 | 1) `gateway-runtime-ops` 全部收口（`18/18`）；2) `release-supply-chain` 推进到 `9/15`（OIDC workflow + provenance + smoke + report + policy 文档 + checkpoint 同步）；3) cross-lang 新增 Java 示例工程与客户端实现（触发/查询/错误处理/超时/重试/幂等）；4) 新增 curl 对照脚本（触发/查询/错误场景）；5) 新增 `docs/protocol/JAVA_GOLDEN_PATH.md` 与 `docs/protocol/cross-lang-acceptance.md`；6) 新增 `scripts/verify_cross_lang.ps1` 与验证报告输出；7) 新增测试 `test_cross_lang_java_assets.py` 与 `test_verify_cross_lang_script.py` 并通过；8) `cross-lang-golden-path/tasks.md` 更新为 `16/16`；9) 定位 run `22437078883` 失败根因并完成本地修复：`datetime.UTC` -> `timezone.utc`，`tomllib` 依赖移除；10) 拉取 release 演练证据 runs `22446541468`、`22447692518`、`22447700064` 及 OIDC run `22449095206`，确认 TestPyPI 阻塞根因收敛到 Trusted Publisher claim 未匹配（`invalid-publisher`）。 |
| 下一待执行 | 1) 触发并跟踪 `test-infra` Task 11.3 远端 CI matrix（3.10/3.11/3.12）复跑，验证本次兼容修复；2) 推进 `release-supply-chain` 外部依赖任务 1.1/1.2 与 3.1/3.2（Trusted Publisher 与分支保护实际配置）；3) 在外部配置完成后复跑 release 链路并收口 Task 4.1/4.2。 |
