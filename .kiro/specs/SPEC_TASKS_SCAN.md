...
| **declarative-binding** | `.kiro/specs/declarative-binding/` | 🟡 三层齐全，进行中（6/26） | 声明式工具绑定（Task 0~5 已完成：契约/schema + Resolver/Registry + HTTP Executor + BindingTool/Ledger） |
...
|------|---|
| 最后更新 | 2026-02-24 |
| 当前批次 | coding loop（codex-work: declarative-binding Task 5） |
| 批次状态 | **进行中（本批）**。`declarative-binding` 已推进至 6/26。 |
| 已完成项 | 1) 完成 Task 5：新增 `BindingTool`（executor 分发、调用计时、结果摘要、异常重抛）；2) 接入 Ledger 记录（tool_name、binding_type、mode、parameters、result_summary、elapsed_ms、status）；3) 新增单测覆盖成功调用、异常记录、摘要截断。 |
| 下一待执行 | `codex-work`：进入 `declarative-binding` Task 6（Skills Loader 扩展，binding 检测与自动注册）。 |
| 验收快照 | 本批验证：`pytest tests/unit/capabilities/test_bindings_tool.py tests/unit/capabilities/test_bindings_executor.py tests/unit/capabilities/test_bindings_schema.py tests/unit/capabilities/test_bindings_credential.py tests/unit/capabilities/test_bindings_http_executor.py tests/integration/test_bindings_http_executor_integration.py -q`（22 passed）+ `ruff check` 通过。 |
| 阻塞项 | 无。 |
| 健康状态 | 正常 |
| 连续无进展轮数 | 0 |
