...
|------|---|
| 最后更新 | 2026-02-26 |
| 当前批次 | codex-gpt-work 执行循环：release OIDC claim 规范化 + 外部阻塞再收敛 |
| 批次状态 | **进行中（阶段性收口）**。gateway-runtime-ops 已完成（18/18）；cross-lang-golden-path 已完成（16/16）；release-supply-chain 维持 10/15，workflow 侧可控改造已完成，阻塞集中于外部 Trusted Publisher 配置。 |
| 已完成项 | 1) 修改并推送 `release.yml`：`release` job 固定 `environment: pypi-release`；2) 触发 run `22449361552` 并确认 OIDC claim 已稳定为 `sub=repo:yeemio/owlclaw:environment:pypi-release`（含 `environment=pypi-release`）；3) 该 run 仍在 TestPyPI `Publish` 返回 `invalid-publisher`，已把证据回写到 `release/tasks.md` 与 `release-supply-chain/tasks.md`；4) 复核 owlhub 生产发布目标仓库状态，`gh repo view yeemio/owlhub` 与 `gh repo view owlclaw/owlhub` 均返回 not found，并已回写 `owlhub/tasks.md` 40.4 证据。 |
...
| 阻塞项 | 1) test-infra Task 11.3：需远端 CI 复跑确认（run `22436813478` 使用旧门槛导致失败，待主分支合并后复验）；2) release/release-supply-chain：主分支 runs `22446541468`、`22447692518`、`22447700064` 仍为旧链路 `TWINE_PASSWORD` 为空导致 TestPyPI `403`；`codex-gpt-work` runs `22449095206`、`22449361552` 已验证 OIDC 链路且 claim 规范化完成，但仍 `invalid-publisher`，当前唯一外部阻塞为 PyPI/TestPyPI Trusted Publisher 映射缺失；3) owlhub Task 40.4：生产凭据/环境所有权外部阻塞，且目标仓库 `yeemio/owlhub` / `owlclaw/owlhub` 当前均不存在（`gh repo view` not found）。 |
| 健康状态 | 正常 |
| 连续无进展轮数 | 0 |
