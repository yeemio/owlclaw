...
**验收标准**：
- [ ] API 版本策略、MCP 版本协商与兼容性规则形成文档化标准并落地到 CI 门禁。
- [ ] 任何协议级 breaking change 都能被自动检测并阻断。
...
**验收标准**：
- [ ] 提供 Java/curl 的协议调用样例并在 CI 或本地脚本中验证通过。
- [ ] 至少 3 个核心场景（触发、查询、审计）可通过协议调用完成。
...
**验收标准**：
- [ ] 形成 `docs/protocol/VERSIONING.md`，覆盖 SemVer 与 breaking change 定义。
- [ ] MCP 初始化协商与 API 版本选择规则一致化（可映射到同一 release policy）。
- [ ] 提供至少 1 个“非破坏升级”与 1 个“破坏升级”流程示例。
...
**验收标准**：
- [ ] API 错误使用 Problem Details（RFC 9457）模型。
- [ ] MCP 错误码映射表与 API 错误域一一对应。
- [ ] 客户端可通过 machine-readable 字段判断 `retryable` 与 `category`。
...
**验收标准**：
- [ ] 每个端点具备请求/响应 schema 与错误示例。
- [ ] 至少 3 个端点完成契约测试。
- [ ] 每个端点可追踪到治理层（budget/rate-limit/ledger）。
...
**验收标准**：
- [ ] `initialize/tools/resources` 路径具备协议回归测试。
- [ ] MCP 与 API 使用统一审计链路（同一 request correlation 维度可追踪）。
- [ ] 明确 non-goal（如 prompts/sampling）并在文档中说明。
...
**验收标准**：
- [ ] OpenAPI breaking check 进入 CI 阻断路径。
- [ ] MCP 契约测试进入 CI 必跑路径。
- [ ] 发布前输出协议变更报告（新增/兼容变更/破坏变更）。
...
- [ ] **FR-1**：版本策略、兼容规则、弃用流程文档化并可执行。
- [ ] **FR-2**：API/MCP 错误语义统一且可机器消费。
- [ ] **FR-3**：最小 API 面（3 端点）可用并通过契约测试。
- [ ] **FR-4**：最小 MCP 面可用并通过协议回归。
- [ ] **FR-5**：Java Golden Path 可执行。
- [ ] **FR-6**：CI 契约门禁生效。
...
- [ ] 版本策略、错误语义、兼容规则三份核心文档齐全且通过评审。
- [ ] API/MCP 对齐矩阵可直接用于审查与回归。

### 9.2 最小协议面

- [ ] API 3 个核心端点完成契约与回归测试。
- [ ] MCP 核心路径完成契约与回归测试。

### 9.3 跨语言可用性

- [ ] Java Golden Path 可执行并有证据。
- [ ] curl 样例可独立复现核心调用链。

### 9.4 发布与安全网

- [ ] CI 契约门禁对 breaking change 生效。
- [ ] 协议变更报告可追溯。

### 9.5 红军评估

- [ ] 至少完成一次“破坏性变更演练”并验证门禁可阻断。
- [ ] 至少完成一次“协议降级/回滚演练”并验证运维手册有效。
