...
- [x] 频率限制评估器实现并通过单元测试
- [x] 熔断约束评估器实现并通过单元测试
- [x] VisibilityFilter 集成所有约束并通过集成测试
- [x] VisibilityFilter 集成到 Agent Runtime 并通过端到端测试

### 9.2 Ledger 完成

- [x] LedgerRecord 数据模型实现并通过迁移
- [x] Ledger 写入接口实现并通过单元测试
- [x] Ledger 查询接口实现并通过单元测试
- [x] Ledger 异步写入队列实现并通过性能测试
- [x] Ledger 集成到 Agent Runtime 并通过端到端测试

### 9.3 Router 完成

- [x] Router 配置加载实现并通过单元测试
- [x] Router 模型选择实现并通过单元测试
- [x] Router 降级链实现并通过单元测试
- [x] Router 集成到 Agent Runtime 并通过端到端测试
- [x] Router 降级事件记录到 Ledger 并验证

### 9.4 集成验证

- [x] 治理层三个组件（VisibilityFilter、Ledger、Router）协同工作
- [x] Agent Run 完整流程通过端到端测试
- [x] 性能指标达标（约束评估 < 10ms，Ledger 异步写入）
- [x] 错误处理和降级策略验证通过
...
|-----------|-----------------|--------|--------|------|
| 预算用完 | 高成本能力隐藏 | 记录过滤决策 | 使用低成本模型 | [x] |
| 非交易时间 | 交易能力隐藏 | 记录过滤决策 | 正常路由 | [x] |
| 超过频率限制 | 能力暂时隐藏 | 记录过滤决策 | 正常路由 | [x] |
| 能力连续失败 | 熔断器打开 | 记录失败和熔断 | 正常路由 | [x] |
| 主模型失败 | 正常过滤 | 记录降级事件 | 降级到备用模型 | [x] |
| Ledger 写入失败 | 正常过滤 | 降级到日志 | 正常路由 | [x] |
