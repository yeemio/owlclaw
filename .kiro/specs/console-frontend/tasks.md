...
**实现**：
- [x] 4.1 实现多维筛选面板（Agent/Capability/时间范围/成本范围/状态）
- [x] 4.2 实现表格视图（DataTable + 分页）
- [x] 4.3 实现详情展开面板（输入/输出/成本/模型/延迟/决策推理）
- [x] 4.4 实现时间线视图（可选）
...
**实现**：
- [x] 5.1 实现 Agent 卡片列表（身份摘要 + 状态指示灯）
- [x] 5.2 实现 Agent 详情页（身份配置 + 记忆统计 + 知识库 + 运行历史）
