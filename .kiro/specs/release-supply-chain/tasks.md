...
- [ ] 4.1 TestPyPI 全链路演练
  ...
  - 演练记录（2026-02-27）：run `22475093887` 再次在 `Publish to TestPyPI` 返回 `HTTP 403 Forbidden`，保持与 `22473801915` 一致的阻塞结论（Trusted Publisher 绑定缺失）
  ...
  - 诊断命令（2026-02-27）：`poetry run python scripts/release_oidc_preflight.py --repo yeemio/owlclaw --run-id 22475093887`（刷新 `docs/release/reports/release-oidc-preflight-latest.md`）
  - 诊断结果（2026-02-27）：preflight 报告状态 `BLOCKED`，仓库侧基线已通过（workflow/protection/ruleset），剩余阻塞为 Trusted Publisher 绑定
- [ ] 4.2 PyPI 正式链路演练
