...
- [ ] 1.3 增加 `trigger_config` 字段（解析后的触发配置）
- [ ] 1.4 单元测试：SkillDescriptor 序列化/反序列化

### Task 2: 自然语言解析器（基础）

- [ ] 2.1 创建 `owlclaw/capabilities/skill_nl_parser.py`
- [ ] 2.2 实现 SKILL.md 模式检测（有无 `owlclaw:` 字段）
- [ ] 2.3 实现 LLM 意图提取（trigger_intent / business_rules）
...
- [ ] 5.1 修改 `skills.py` 的 `_parse_skill_file()` 增加双模式路由
- [ ] 5.2 集成 skill_nl_parser + trigger_resolver
