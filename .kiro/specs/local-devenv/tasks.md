...
- [x] **Task 6**: scripts/test-local.sh + scripts/test-local.ps1
  - [x] 6.1 创建 `scripts/test-local.sh`：启动 test compose → 等待 healthcheck → 运行 pytest → 停止
  - [x] 6.2 创建 `scripts/test-local.ps1`：Windows 等价版本
  - [x] 6.3 支持参数：`--unit-only`（跳过集成测试）、`--keep-up`（测试后不停止服务）
  - _Requirements: AC-3_
