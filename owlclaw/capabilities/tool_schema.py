...
def _normalize_parameter_field(value: Any) -> tuple[dict[str, Any] | None, str | None]:
    ...
    for name, spec in value.items():
        ...
        if isinstance(spec, str):
            ...
            if not isinstance(type_name, str) or type_name.strip().lower() not in _SIMPLE_TYPES:
                return None, f"unsupported simplified parameter type: {type_name!r}"
            schema = dict(spec)
            schema["type"] = type_name.strip().lower()
        else:
