...
| `workflow_executor.py` | 真正执行 mailbox 动作：在对应 worktree 中直接调用 `codex exec` 完成审校/收口类任务 | `poetry run python scripts/workflow_executor.py --agent review --once` | 本地开发使用 |
...
6. `workflow_executor.py` 是执行层：它读取 mailbox 后直接调用非交互 `codex exec`，结果写入 `.kiro/runtime/executions/` 并回写 ack。
